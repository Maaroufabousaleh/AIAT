...
- [x] Add the consolidated `aiat.project-evidence-package.v1` project API/dashboard read model, deterministic fixture, and operator-only durable snapshot path covering repository, documents, tests, security, deployment, cost, approvals, flow, worker, artifact, and audit evidence; the deterministic core/resolver/fixture batch is reviewed and committed as `a44a1aa`, package-level workflow exports are isolated in `d0472af`, and the isolated API/snapshot/policy route group is committed in `cbf00d9`; dashboard wiring and live provider/worker artifact generation remain separate.
