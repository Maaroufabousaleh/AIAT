...
- Add stale/offline/partial/denied/conflict/rollback designs. System
  ...
  stale/partial/conflict endpoint failures; the Flows list now exposes an
  explicit stale endpoint failure and retryable first-load/offline state;
...
- Deep-link statuses to evidence, traces, decisions, and recovery; the CEO
  ...
  covered by focused 1/1 source-built tests; Flows list recovery is also covered
  by `flows-states.spec.ts`; broader stale/offline recovery and
