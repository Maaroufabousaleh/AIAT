...
- [x] Preserve identity-resource last-known records on refresh failure, label the stale state, abort obsolete refreshes, expose semantic table/action controls with 44px targets, and expose a retry action; `e2e/identity-states.spec.ts` passes the authenticated failure → retained rows → successful recovery and accessibility assertions 1/1 without rendering sensitive fields (`46eccee`, `651ad11`). Provider/live identity evidence remains separate.
