...
- Add stale/offline/partial/denied/conflict/rollback designs. System
  ...
  combined governance read surface, and System Control now expose explicit
  stale/partial/conflict endpoint failures and retryable first-load/offline
...
- Deep-link statuses to evidence, traces, decisions, and recovery; the CEO
  ...
  list read-state recovery are also covered by focused 1/1 source-built tests;
  broader stale/offline recovery and golden-path coverage remain.
