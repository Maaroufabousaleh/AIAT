...
- [x] Add the bounded `aiat.evidence-detail.v1` dashboard read model (`8fefc8b`, trace extension `c8505eb`) for project, flow, flow-instance, worker, worker-run, credential, dead-letter, runtime, and trace citations; the proxy allow-lists scalar fields and backend paths, strips nested payloads/trace items, and preserves identity-only behavior for unsupported kinds. Source-built focused coverage passes 3/3; broader detail kinds and stale/offline recovery remain open.
