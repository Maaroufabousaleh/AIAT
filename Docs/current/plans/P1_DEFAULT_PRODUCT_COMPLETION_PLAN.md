...
- [x] Add the bounded `aiat.evidence-detail.v1` dashboard read model (`8fefc8b`, trace extension `c8505eb`, model/integration expansion and scalar hardening `dc50719`, recovery regression `5357166`) for project, flow, flow-instance, worker, worker-run, credential, dead-letter, runtime, trace, model, and integration citations; the proxy selects matching list/catalogue records, allow-lists scalar fields and backend paths, bounds values, strips nested payloads/trace items/configuration/profile bindings, and preserves identity-only behavior for unsupported kinds. Source-built focused coverage passes 6/6, including temporary-detail-unavailable identity recovery; broader detail kinds and stale/offline recovery remain open.
