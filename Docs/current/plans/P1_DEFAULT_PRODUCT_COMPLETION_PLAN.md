...
- [x] Preserve identity-resource last-known records on refresh failure, label the stale state, abort obsolete refreshes, and expose a retry action; `e2e/identity-states.spec.ts` passes the authenticated failure → retained rows → successful recovery path 1/1 without rendering sensitive fields (`46eccee`). Provider/live identity evidence remains separate.
