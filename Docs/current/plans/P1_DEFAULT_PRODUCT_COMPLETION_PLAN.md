...
- [x] Export and reconcile the deterministic `aiat.model-profile-catalogue.v1` runtime/profile catalogue with versioned provider/model/capability/price/privacy/region data; retained 2026-08-18 live evidence observes complete 93 approved covered versions out of 93 persisted versions with no findings after exact unreferenced smoke-fixture cleanup, while the active catalogue now has 92 entries after removing retired Groq `llama-3.3-70b-versatile` and retaining `openai/gpt-oss-120b`.
...
- [x] Add role-scoped `aiat.executive-action.v1` write envelopes: CFO `/executive/actions/cfo/model-overrides` creates the existing durable override request, CTO `/executive/actions/cto/worker-runs` delegates to canonical governed worker dispatch, and CEO `/executive/actions/ceo/privileged-actions` delegates to the audited privileged gate. The dashboard has operator-authenticated proxies and a typed confirmation panel for all three; the retained local profile evidence is complete at 93/93 with no findings, while the active registry now has 92 entries after the retired Groq model removal. Provider-specific live recovery, broader governance forms, and chaos evidence remain open.
...
- [x] Preserve identity-resource last-known records on refresh failure, label the stale state, abort obsolete refreshes, expose semantic table/action controls with 44px targets, and expose a retry action; the shared page also exposes named main/status/metadata/table regions, explicit busy state, decorative-icon suppression, and a distinct 401/403 access-denied region that removes misleading Refresh/Retry actions while preserving already loaded metadata-only rows. `e2e/identity-states.spec.ts` passes the authenticated failure → retained rows → successful recovery and denied-access assertions 2/2 without rendering sensitive fields (`46eccee`, `651ad11`, `a260e04`, `0974434`), while `identity-resource-matrix.spec.ts` covers all nine shared identity/mail routes 9/9 with safe fixture metadata (`485dfd2`). Provider/live identity evidence remains separate.
...
- [x] Add Hiring Board access-denied recovery: a 401/403 worker read now exposes a named denial region, removes Refresh/Retry, registration, evaluation, activation/deactivation, drain, and deletion controls, and preserves only last-known worker rows after access is lost; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`553f196`).
...
- [x] Add CEO Live Feed access-denied recovery: 401/403 history, SSE, and composer responses now expose a named denial region, preserve only previously loaded messages, invalidate in-flight stream callbacks, and hide reconnect/retry, copy/clear/filter, and composer controls; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`a3cbd99`).
...
- [x] Add CEO Command Center chat access-denied recovery: 401/403 history, SSE, and message-submission responses now expose a named access-status region, retain any loaded transcript as read-only context, invalidate in-flight stream callbacks, and hide Clear/retry, quick commands, composer, and confirmation controls; source-built history-denial, message-denial, and existing stale-recovery coverage passes 3/3 (`038d5f2`). Native/live ACL and full WCAG/native-Linux evidence remain separate.
...
- [x] Add Governance access-denied recovery: a 401/403 from any combined read now exposes a named denial region, removes Refresh/Retry and all executive action forms, and preserves only last-known read context after access is lost; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`888fde3`).
...
- [x] Add System Control access-denied recovery: 401/403 status reads now expose a named denial region, remove Refresh/Retry and shutdown/resume/schedule mutations, and preserve only last-known read context after access is lost; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`14968d4`).
...
- [x] Add Projects list access-denied recovery: 401/403 paired project/active-flow reads and create/archive/delete mutations now expose a named denial region, retain only previously loaded definitions as read-only text, clear selection, and hide Refresh/Retry, New Project, filters, sorting, archive, and delete controls; source-built stale, first-load-denial, retained-read-denial, and mutation-denial coverage passes 4/4 (`17d25b0`).
...
- [x] Add Project evidence package access-denied recovery: canonical package-read 401/403 responses expose a named denial region, retain a previously loaded package as read-only context, hide Refresh/Retry and package controls, and preserve safe Back to project navigation; source-built stale, initial-denial, and retained-read denial coverage passes 3/3 (`00f81b5`).
...
- [x] Add Project Detail access-denied recovery: canonical project reads and workflow/mutation 401/403 responses expose a named denial region, retain only the last-known project header as read-only context, clear pending interaction state, and hide refresh, retry, tabs, panels, and workspace/workflow/flow/context/evidence/repository/approval/transition mutations; source-built transient, initial-denial, retained-header-denial, and workflow-mutation-denial coverage passes 4/4 (`0671eaa`).
...
- [x] Harden the dedicated evidence-detail page accessibility baseline with a named page/canonical-citation region, a semantic bounded-detail region with an explicit `aria-busy` refresh state, decorative-icon suppression, and 44px CEO-chat/canonical-link/Refresh targets; the existing `e2e/dashboard-evidence-detail.spec.ts` covers these semantics and target sizes in the expanded 11/11 suite (`32f3a76`, `23e2db9`). This remains a focused page-level baseline, not the full WCAG/native-Linux audit.
- [x] Add Evidence Detail access-denied recovery: bounded scalar-read 401/403 responses expose a named denial region, retain any last-known scalar projection as read-only, hide Refresh, and preserve citation identity plus safe CEO/owning-section navigation; source-built scalar, stale, initial-denial, and retained-read coverage passes 11/11 (`23e2db9`).
...
- [x] Add Tools catalogue access-denied recovery: 401/403 responses now expose a named denial region, retain only previously loaded tool metadata, and hide refresh, retry, search, grouping, expansion, and copy controls while leaving retained tables/details read-only; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`b418f8a`).
...
- [x] Add dead-letter queue access-denied recovery: 401/403 read or replay responses now expose a named denial region, retain only previously loaded messages, clear selection/replay state, and hide refresh/retry, filters, selection, and replay controls while retaining read-only envelope inspection; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`e6ab3a1`).
...
- [x] Add Credentials access-denied recovery: a 401/403 read now exposes a named denial region, preserves only previously loaded redacted metadata, and hides Refresh/Retry, creation, deletion, placeholder-copy, selection, and audit navigation controls; creation and bulk mutations also fail closed when authorization is lost. Source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`982c9c0`).
...
- [x] Add Metrics access-denied recovery: 401/403 responses from any of the six Prometheus query families now expose a named denial region, preserve only previously loaded series, and hide refresh/retry, time-range, and reconnect controls; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`b64b15e`).
...
- [x] Add Flows list access-denied recovery: 401/403 reads and deletes now expose a named denial region, preserve only previously loaded definitions as read-only text, and hide Refresh/Retry, New Flow, search/status filters, selection, editing, and deletion controls; source-built stale, first-load-denial, retained-read-denial, and mutation-denial coverage passes 4/4 (`3108b02`).
...
- [x] Add flow-editor access-denied recovery: 401/403 flow reads and saves now expose a named denial region, preserve only the last successfully loaded canvas as read-only, and hide Refresh/Retry, palette, editing, undo/redo, and save controls; source-built stale, first-load-denial, retained-read-denial, and mutation-denial coverage passes 4/4 (`392d264`).
- [x] Add new-flow builder access-denied recovery: 401/403 governed worker/profile/template reads and create/readiness mutations now expose a named access-status region, retain the current draft canvas as read-only context, and hide template application, the palette, node configuration, validation, activation, and creation controls while preserving Back to flows; source-built initial-catalogue, create-denial, and readiness-denial coverage passes 3/3 (`b07299b`). Native/live ACL and full WCAG/native-Linux certification remain separate.
...
- [x] Add Container Logs access-denied recovery: 401/403 SSE responses now expose a named denial region, preserve only previously loaded lines, invalidate obsolete stream generations, and hide load/retry, filter, clear, copy, and download controls; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`156597c`).
...
- [x] Add Agent Streams access-denied recovery: 401/403 history or SSE responses now expose a named denial region, preserve only previously loaded messages, invalidate in-flight stream callbacks, and hide reconnect/retry, filter, pause, clear, and copy controls; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`118ff18`).
...
- [x] Add a distinct System Visualization access-denied state: 401/403 hierarchy responses now expose a named denial region with operator guidance and a 44px return link without a misleading Retry action; source-built healthy/partial/offline/denied coverage passes 4/4, and partial notices identify each failed source (`db898e7`).
...
- [x] Add PM integrations access-denied recovery: a 401/403 read response now exposes a named denial region, removes Refresh/Retry and lifecycle-plan generation/review/approval/apply mutations, and preserves only last-known reconciliation context after access is lost; source-built stale, first-load-denial, and post-read-denial coverage passes 3/3 (`7373360`).
...
- [x] Add explicit System Overview access-denied recovery: classify 401/403 control-plane or metrics responses separately from outage states, identify denied sources in a named status region, retain available overview values as read-only context, and hide retry/first-run seed actions until authorization returns; the source-built local 403 fixture passes 1/1 (`b0ab779`). Native/live ACL and full WCAG/native-Linux certification remain separate.
- [x] Hide decorative icons in the shared `EmptyState` primitive with `aria-hidden`; the offline System Overview fixture asserts the unavailable state does not expose the status icon to assistive technology (`24be4ba`).
- [x] Hide decorative severity icons in the shared `ErrorBanner` primitive with `aria-hidden`; the System Visualization partial-state fixture asserts the rendered warning icon is not exposed to assistive technology (`29b700c`).
...
- [x] Add the bounded `aiat.evidence-detail.v1` dashboard read model (`8fefc8b`, trace extension `c8505eb`, model/integration expansion and scalar hardening `dc50719`, recovery regression `5357166`, tool catalogue extension `ec8cf67`, artifact/usage authority `2ca5f3d`, stale-refresh retention `6c52552`) for project, flow, flow-instance, worker, worker-run, credential, dead-letter, runtime, trace, model, integration, tool, artifact, and usage citations; the proxy selects matching list/catalogue/manifest records or operator-authenticated artifact/usage reads, allow-lists scalar fields and backend paths, bounds values, strips nested payloads/trace items/metadata/pricing/resource/configuration/profile bindings/tool schemas/credential requirements, retains the last successful scalar projection through failed refreshes, exposes a retry control, and preserves identity-only behavior for unsupported kinds. Source-built focused coverage passes 11/11, including artifact/usage scalar checks, stale-refresh identity recovery, and initial/retained denial states (`23e2db9`); broader stale/offline recovery remains open.
- [x] Stabilize the Governance and Projects refresh effects (`60839f4`): memoize loaders with `useCallback`, keep last-known snapshots in refs for failure classification, and preserve retry/denial behavior without changing the API contract. Dashboard `npm run lint`, `npm run typecheck`, and `npm run build` pass with no lint warnings. The production-server source-built suites pass 7/7 (`governance-states.spec.ts` 3/3 and `projects-states.spec.ts` 4/4). Two local seven-test attempts could not load their fixture-backed data and are classified `TEST-HARNESS/EXECUTION INVALID`; they are excluded from release evidence and do not advance the UI or global release gates.
