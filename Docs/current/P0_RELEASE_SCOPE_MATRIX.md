...
cleanup hardening in the maintained status surfaces. `0d821bd` additionally
verifies the pinned image platforms, preserves the governed task prompt for
the live request without retaining payloads, and validates the neutral
OpenHands/OpenCode comparison record. The registration
...
> `9b5aa6f97bc7d12cdfee231bfb43afe8ff8a69bc`, which includes the direct/nested
...
> incomplete, and scrubs credential-like environment variables from task-test
> subprocesses. The table's earlier tip references are retained as
