...
| Machine-readable release ledger | PASS (50/50 static aggregation; native live-ledger gate `4d7a495`); BLOCKED (current unconfigured live profile and retained configured profile) | `uv run --isolated python scripts/check_release_ledger.py --json` reports 50/50 static checks passing, two pending worker security findings-review items, and `NO-RELEASE`. The current unconfigured 68-check `--live --json` snapshot records 53 pass/15 blocked/0 fail with four pending items and is retained at [`provenance/release_ledger_live_current.json`](../../mas/docs/provenance/release_ledger_live_current.json); the configured loopback 64-check snapshot remains at [`provenance/release_ledger_live.json`](../../mas/docs/provenance/release_ledger_live.json) with 59 pass/5 blocked. The native preflight and Firecracker readiness child are included in the live aggregate and never expose credentials; licence metadata remains non-gating. |
