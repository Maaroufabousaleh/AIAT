...
- The host-safe local Compose sweep (`38a8ab7`, evidence refresh `a415a1b`, observed
  `2026-08-19T06:23:37Z`) uses published loopback endpoints through
  ...
  profile does not change `NO-RELEASE`.
...
- The AIAT credentials-manager live certificate (`12ba7c7`, scalar evidence
  ...
  refreshed at 2026-08-19T04:19:33Z with evidence commit `53660d3`; release
...
| Host-safe local Compose release-ledger sweep | BLOCKED/NO-RELEASE (79/85 pass, 6 blocked, 0 fail) | Implementation `38a8ab7` adds `check_release_ledger.py --live --compose-local` to map host-side children to published loopback endpoints while preserving explicit overrides. Scalar evidence refresh `a415a1b` is [`provenance/release_ledger_live_compose_local_current.json`](../../mas/docs/provenance/release_ledger_live_compose_local_current.json), observed at 2026-08-19T06:23:37Z; four pending evidence items and the six native/image/sandbox/mail/self-improvement blockers remain. |
| Native release-host preflight | BLOCKED (current local WSL2 host) | The opt-in `uv run --isolated python scripts/check_release_environment.py --require-native-linux --json` check was refreshed at 2026-08-19T04:15:06Z against revision `39cb545`. The current WSL2 result is retained at [`provenance/native_release_preflight.json`](../../mas/docs/provenance/native_release_preflight.json) with safe blockers for host identity, `runsc`, missing deployment-supplied immutable image refs, and dirty state. It is a prerequisite diagnostic only; native network, image/SBOM, scan, recovery, and provider evidence remain open, and licence metadata is non-gating. |
