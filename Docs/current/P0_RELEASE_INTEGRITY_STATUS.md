...
| Durable in-flight worker-version pinning | PASS (local Compose Postgres; complete bundle/multi-host pinning open) | Commits `dbf6d10` and `8bb0a91`; `uv run --isolated pytest scripts/tests/test_check_worker_version_pinning_postgres.py -q` plus focused Ruff pass. The checker creates shell-v1/v2, adapter-v1/v2, and bundle-v1/v2 records, keeps a version-one `RUNNING` run pinned to its shell/adapter/steward after registry pointers advance, creates a version-two queued run, reopens Postgres, reads labels and states back, emits no payload, and cleans the reserved graph. The current `worker_runs` schema has no `skill_bundle_id`; complete bundle pinning, live worker dispatch, real host-loss/split-brain, gVisor/Firecracker, live worker/provider, and licence metadata gates remain separate |
| Deterministic worker placement policy | PASS (static/unit/fixture; durable multi-host scheduler certified) | Commit `db22e60`; `uv run --isolated ruff check packages/mas-core/mas_core/worker_registry/placement.py packages/mas-core/tests/test_worker_placement.py scripts/check_worker_placement.py scripts/tests/test_check_worker_placement.py` and `PYTHONPATH=packages/mas-core:scripts uv run --isolated pytest packages/mas-core/tests/test_worker_placement.py scripts/tests/test_check_worker_placement.py -q` pass. The fixture checker covers eligible-host selection, blocked capacity/constraint filtering, and duplicate-host fail-closed behavior over explicit snapshots; the predicate enforces readiness/lease, labels, capabilities, sandbox/isolation, and slot/memory/GPU capacity without mutation or dispatch. Scheduler selection/fallback is certified by `d9917f8`; host-loss/split-brain, gVisor/Firecracker, and licence metadata gates remain separate. Evidence is [`worker_placement_contract.json`](../../mas/docs/provenance/worker_placement_contract.json) |
| Durable authenticated worker-host registry | PASS (local Compose Postgres; host-loss/dispatch open) | Commit `500fc57`; migration `0037_worker_host_registry`, focused host-registry unit/checker tests, Ruff, and the live Compose checker pass. The certificate stores only a token digest, rejects wrong-token registration/heartbeat, renews an AIAT-owned lease, projects redacted placement snapshots, survives connection reopen, exposes an expired lease as invalid, and cleans the reserved host to zero rows. Durable host recovery, host-loss/split-brain, gVisor/Firecracker, live worker/provider, and licence metadata gates remain separate; capacity reservation/commit is certified by `232c0bb` and deterministic scheduler integration by `d9917f8` |
| Durable worker-host capacity reservations | PASS (local Compose Postgres; host-loss/dispatch open) | Commit `232c0bb`; migration `0038_worker_host_reservations`, focused reservation unit/checker tests, Ruff, and the live Compose checker pass. The ledger requires a READY host lease, serializes per-host capacity checks, rejects over-capacity requests, replays an idempotent key, records commit/release/expiry transitions, reads scalar capacity after connection reopen, and cleans all reserved rows. Deterministic multi-host selection/fallback is certified separately by `d9917f8`; durable host-loss/split-brain recovery, worker dispatch, gVisor/Firecracker, live worker/provider, and licence metadata gates remain separate. Evidence is [`worker_host_reservations_postgres_evidence.json`](../../mas/docs/provenance/worker_host_reservations_postgres_evidence.json) |
| Durable worker-host multi-host scheduler | PASS (local Compose Postgres; host-loss/dispatch open) | Commit `d9917f8`; `host_scheduler.py`, focused scheduler unit/checker tests, Ruff, and the live Compose checker pass. The scheduler ranks registry snapshots, falls back after a row-locked preferred-host capacity rejection, replays a globally idempotent schedule key, filters draining/unleased hosts, reports blocked full capacity, survives connection reopen, and cleans its reserved fixtures without worker/provider dispatch. Durable host-loss/split-brain recovery, live worker dispatch, gVisor/Firecracker, and licence metadata gates remain separate. Evidence is [`worker_host_scheduler_postgres_evidence.json`](../../mas/docs/provenance/worker_host_scheduler_postgres_evidence.json) |
