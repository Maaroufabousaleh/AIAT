...
| Gateway worker/mail-edge local composition | PASS (local scalar + durable dual-Postgres composition; live provider sources open) | Commits `6ebb12c` and `fa42284`; the scalar fixture and `check_gateway_worker_mail_edge_postgres.py --json` pass with the real `GatewayWorkerAdapter`/`WorkerRunController`, exact fixture provider/model usage, normalized delivery/verified-webhook/bounce observations, independent worker/identity Postgres reopen, payload-free cross-store correlation, and scoped cleanup. The durable certificate makes no HTTP-ingress, external provider, selected live worker, recovery, sandbox, or host-runtime claim. Evidence is [`gateway_worker_mail_edge_fixture.json`](../../mas/docs/provenance/gateway_worker_mail_edge_fixture.json) and [`gateway_worker_mail_edge_postgres_evidence.json`](../../mas/docs/provenance/gateway_worker_mail_edge_postgres_evidence.json); licence metadata remains separate/not-gated. |
