...
- The live profile was exercised after the local Compose recreation with the
  configured loopback orchestrator and tool-service endpoints. The fresh
  2026-08-11 64-check profile records 59 passes, zero failures, and five
  externally blocked probes with a bounded 60-second child-check timeout; the
  authenticated worker reconciliation child matches 39/39 persisted defaults
  with zero missing rows or binding mismatches, and the trace/tool children
  ...
  outbound relay, and self-improvement signal source. These are current
...
| Machine-readable release ledger | PASS (static aggregation; committed `eff4eef`); BLOCKED (fresh live profile with configured loopback endpoints) | `uv run --isolated python scripts/check_release_ledger.py --json` reports 48/48 static checks passing, two pending worker security findings-review items, and `NO-RELEASE`; the fresh configured 64-check `--live --json` snapshot (commit `8fc1502`) records 59 pass/5 blocked with zero failures and never exposes credentials. The authenticated worker-reconciliation child passes 39/39 persisted defaults with zero missing rows or binding mismatches, and the trace/tool children read back one API/native transport pair plus one project-usage/native tool pair. Operator-selected worker certification, immutable image identity, gVisor, outbound relay, and self-improvement source remain blocked; four pending evidence items remain. Each child checker is bounded by the ledger timeout; the object-store child invokes `--compose-local` to execute the checked-in 8/8 MinIO probe inside the private network. The separate local `check_network_boundary.py --live --json` matrix passes all 11 runners; the metric-series scrape still passes when a live endpoint is configured after Prometheus histogram `_created` normalization. |
