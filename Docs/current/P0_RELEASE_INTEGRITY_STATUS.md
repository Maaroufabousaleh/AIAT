...
| Hierarchy communication-policy overlay | PASS (source type/lint/build; E2E spec/live image pending) | Commit `8b7d9f1`; `npm run typecheck`, focused ESLint, and `npm run build` pass for the dashboard; `app-operations.spec.ts` covers sender-role selection plus allowed/denied path labels, but the new interaction has not yet executed successfully against a current image. The running Compose dashboard image predates the commit, and a rebuild is blocked by unreadable `.tmp-*` Docker-context paths, so live UI evidence remains open |
