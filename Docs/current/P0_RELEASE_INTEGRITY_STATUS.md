...
- The bounded trace incident projection/checker (`c357fdf`) and its
  operator-only API/generated-contract/dashboard deep-link boundary
  (`b4b7cef`) are covered by deterministic core/script/API/dashboard tests and
  remain a read-only descriptive summary; they do not turn partial coverage,
  incident findings, or licence metadata into an activation or release gate.
