...
| API/protocol contract export | PASS (static/preparatory) | Commits `66b8690`, `f2b0961`, and `fd61456`; `uv run --isolated python scripts/check_api_contract.py --json` — 233 OpenAPI paths/130 schemas and checked-in `aiat.v1` protocol schema match runtime/provenance hashes, including native trace-evidence source, SLO/capacity, evidence-package, self-improvement outcome-action, artifact-bundle, and read-back-action fields |
| Python SDK contract generation | PASS (static/unit/preparatory) | `uv run --isolated python scripts/generate_python_api.py --check`; `pytest packages/mas-api-sdk/tests`; 130 models and 266 operations match OpenAPI |
| Company timezone propagation | PASS (tool-service unit; dashboard/Compose follow-up) | Commit `8bcff1a`; `uv run --isolated pytest apps/tool-service/tests/test_time.py -q` proves the `time_now` tool follows a valid company IANA zone and fails back to UTC on invalid input without changing UTC persistence. Dashboard/Compose/default-wrapper propagation remains a separate review because the current dashboard typecheck has an unrelated `ErrorBanner` prop failure |
