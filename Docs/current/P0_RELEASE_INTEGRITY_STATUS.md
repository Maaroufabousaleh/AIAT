...
The maintained dashboard evidence now also includes Flows list stale/retry recovery (`a0faf5b`, source-built `flows-states.spec.ts` 1/1) and Container Logs stale/retry recovery (`280d363`, source-built `logs-states.spec.ts` 1/1); native/live flow/log evidence remains separate from these preparatory P1 results.
