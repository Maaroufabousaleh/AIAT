...
- `scripts/check_release_environment.py --json` (inputs committed as
  ...
  exists.
- `scripts/check_docs_index.py --json` passes the canonical target, twelve current
...
| Durable in-flight worker-version pinning | PASS (local Compose Postgres; complete bundle pinning certified, multi-host open) | Commit `6a10b0e`; migration `0040_worker_run_skill_bundle_pin`; focused controller/checker tests and live checker pass. The checker creates shell-v1/v2, adapter-v1/v2, and bundle-v1/v2 records, keeps a version-one `RUNNING` run pinned to its shell/adapter/bundle/steward after registry pointers advance, creates a version-two queued run, reopens Postgres, reads labels and states back, emits no payload, and cleans the reserved graph. Live worker dispatch, real host-loss/provider recovery, gVisor/Firecracker, multi-host execution, and licence metadata gates remain separate |
...
| Concurrent multi-host native worker execution | PASS (local Compose Postgres; two-host native fixture; independent-host/runtime open) | Commit `f9c717b`; focused checker tests, Ruff, and `check_worker_multi_host_execution_postgres.py --json` pass. Two distinct worker-plane host records receive committed reservations and execute two queued runs concurrently through `WorkerHostExecutor`; both runs claim and succeed, host lease-generation/current-lease equality is read back, both bindings/reservations release, two usage rows, two artifacts, three native spans per trace, and payload-free coverage survive a Postgres reopen, and scoped cleanup leaves zero fixture rows. Independent deployed hosts, selected model-backed dispatch, gVisor/Firecracker, providers, host-loss/split-brain, and recovery remain separate; licence metadata is not a gate. Evidence is [`worker_multi_host_execution_postgres_evidence.json`](../../mas/docs/provenance/worker_multi_host_execution_postgres_evidence.json) |
| Fenced worker-host loss and queued-run recovery | PASS (local Compose Postgres; scoped host-loss/requeue/reassignment; independent-host/runtime open) | Commit `893293a`; focused recovery/binding/executor tests, Ruff, and `check_worker_host_loss_queue_recovery_postgres.py --json` pass. A host-filtered reconciliation fences one expired worker host and reservation, the canonical Worker Run lease loop requeues the expired claim, the stale host executor is rejected before dispatch, and `WorkerRunHostBindingService.reassign_after_host_loss()` moves the queued binding to host B for a native retry at attempt two. The certificate reads back `SUCCEEDED`, released binding/reservation, one usage row, one artifact row, three native spans, payload-free coverage, Postgres reopen, and zero fixture rows. Independent deployed hosts, gVisor/Firecracker, providers, and provider-backed recovery remain separate; licence metadata is not a gate. Evidence is [`worker_host_loss_queue_recovery_postgres_evidence.json`](../../mas/docs/provenance/worker_host_loss_queue_recovery_postgres_evidence.json) |
| Selected model-resolution worker-host execution | PASS (local Compose Postgres; deterministic profile/snapshot propagation; external provider open) | Commits `6cef1b8` and `9a7db70`; focused checker/executor tests, Ruff, and `check_worker_host_model_resolution_postgres.py --json` pass. The certificate creates an approved Model Profile/version, resolves it through the deterministic resolver, persists the snapshot, rejects mismatched snapshot references before claim, carries valid requested/resolved references through a committed worker-host run, and reads back `aiat_gateway` mode, exact provider/model usage, terminal evidence, released binding, payload-free trace coverage, Postgres reopen, and zero fixture rows. Provider/model identifiers are local fixtures; external provider calls, independent hosts, gVisor/Firecracker, provider-backed recovery, and licence metadata remain separate/not-gated. Evidence is [`worker_host_model_resolution_postgres_evidence.json`](../../mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json) |
...
| Gateway worker/mail-edge local composition | PASS (local fixture composition; durable/live provider sources open) | Commit `6ebb12c`; `check_gateway_worker_mail_edge_fixture.py --json`, its focused test, gateway adapter/controller tests, worker-trace/mail-edge evaluator tests, and targeted Ruff pass. The real `GatewayWorkerAdapter` and `WorkerRunController` produce exact fixture provider/model usage; a bounded scalar worker/integration projection is joined with verified delivered and bounced observations under `aiat.worker-mail-edge-coverage.v1`. The report is payload-free and non-mutating, with no external provider call, durable provider callback/read-back, live worker dispatch, sandbox execution, or host certification. Evidence is [`gateway_worker_mail_edge_fixture.json`](../../mas/docs/provenance/gateway_worker_mail_edge_fixture.json); licence metadata remains separate/not-gated. |
