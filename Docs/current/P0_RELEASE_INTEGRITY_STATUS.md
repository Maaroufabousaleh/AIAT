...
| Selected worker-run readiness health guard | PASS (fixture + mocked read-only live diagnostic) | Commit `2eea80a`; `test_check_worker_run_readiness.py` proves a 503/unavailable selected-worker health response becomes `read_worker_health_unavailable` rather than an unqualified not-checked result. No activation, dispatch, reservation, or licence gate is added |
...
5. Certify each default runtime/adapter against the installed lock and live
   ...
   evidence. Commit `2eea80a` also makes unavailable or malformed selected-worker
   health responses explicit `read_worker_health_unavailable` blockers rather
   than silently treating health as not checked.
