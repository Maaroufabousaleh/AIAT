...
- Mail-edge observation contracts/checker (`85369fe`) and identity-service
  persistence/projection (`cfafe38`) pass the deterministic payload-free
  delivery/webhook/bounce fixture and focused service suites; provider ingress
  certification, selected worker live read-back, and complete mail-span
  evidence remain open and non-gating.
...
| External-account action policy and mail correlation | PASS (static/API/unit, preparatory P1) | Commits `f577675` and `cfafe38`; `PYTHONPATH=apps/identity-service uv run --isolated pytest apps/identity-service/tests/test_identity_service.py apps/identity-service/tests/test_provider_adapters.py -q`; versioned action taxonomy, closure human-approval/session-revocation path, safe delivery-attempt trace/span persistence, and normalized provider-event migration `0003_mail_edge_observations` pass; provider ingress certification, live outage, selected worker, and complete mail-edge evidence remain open |
