...
- Request-level trace propagation is now verified for the orchestrator API,
...
- The live profile was exercised after the local Compose recreation. The
  2026-08-10 64-check profile records 59 passes, zero failures, and five
  externally blocked probes with a locally configured orchestrator URL and a
  bounded 60-second child-check timeout; the standalone local network matrix
  is green and retained separately. The profile retains four pending evidence
...
| HTTP/message/trace-evidence suite | PASS (local transport + tool read-back; broader sources open) | `uv run --isolated pytest packages/mas-core/tests/test_tracing.py packages/mas-core/tests/test_phase4_5.py packages/mas-core/tests/test_trace_evidence.py packages/mas-core/tests/test_native_trace_spans.py apps/orchestrator-api/tests/test_trace_propagation.py apps/orchestrator-api/tests/test_trace_evidence.py apps/message-router/tests/test_trace_propagation.py apps/tool-service/tests/test_trace_propagation.py apps/tool-service/tests/test_usage_native_spans.py packages/mas-tools-sdk/tests/test_smoke.py -q`; `uv run --isolated python scripts/check_trace_evidence.py --json`, `scripts/check_native_trace_spans.py --json`, and `scripts/check_live_tool_trace.py --json` pass deterministic contracts; local live transport and pure-tool read-back are retained separately; model-backed worker, mail-edge, and retention probes remain open |
...
| Project evidence package | PASS (static/unit/API/dashboard, preparatory P1) | `uv run --isolated pytest packages/mas-core/tests/test_evidence_package_runner.py packages/mas-core/tests/test_workflow_scaffold.py apps/orchestrator-api/tests/test_projects.py -q`; `uv run --isolated python scripts/check_project_evidence_package.py --json`; dashboard `npm run typecheck`; the `aiat.project-evidence-package.v1` read model and operator-only durable snapshot group repository/test/security/deployment/cost sources without changing completion authority; live provider/worker artifact generation remains open |
...
| Governed asynchronous flow-task binding | PASS (deterministic contract fixture, preparatory P1) | `uv run --isolated pytest packages/mas-core/tests/test_flow_worker_binding.py apps/orchestrator-api/tests/test_flows.py -q`; `uv run --isolated python scripts/check_flow_worker_binding.py --json` proves queued/claimed/running Worker Runs keep their task active, terminal states settle, parallel bindings remain copy-on-write, safe retry re-enters governed dispatch, and unknown states fail closed; the retry API preserves prior executions as `SUPERSEDED`; live canary/recovery remains open |
...
| Machine-readable release ledger | PASS (static aggregation); BLOCKED (live profile with local URL) | `uv run --isolated python scripts/check_release_ledger.py --json` reports all 48 static checks passing with pending evidence and `NO-RELEASE`; the corrected configured `uv run --isolated python scripts/check_release_ledger.py --live --json` profile now includes the bounded runtime benchmark, model-profile, trace-evidence, and private-network object-store, and container runtime-import children alongside the transport/tool probes, records 59 pass/5 blocked across 64 checks, and never exposes credentials. Each child checker is bounded by the ledger timeout; the object-store child invokes `--compose-local` to execute the checked-in 8/8 MinIO probe inside the private network. The separate local `check_network_boundary.py --live --json` matrix passes all 11 runners; the metric-series scrape still passes when a live endpoint is configured after Prometheus histogram `_created` normalization. |
| Release environment manifest | PASS (secret-safe static identity) | `uv run --isolated python scripts/check_release_environment.py --json` emits `aiat.release-environment.v1` with thirteen input hashes, tool identities, environment-presence flags, and deterministic digest `f220dee05c1442cd88b3ee9d74bc579cf87fab68b6553bce2aa1a88de8c6336b` without printing values or credentials; the current WSL evidence records 389 changed paths and dirty state is recorded, not hidden |
