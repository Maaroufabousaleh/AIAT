...
| Machine-readable release ledger | PASS (55/55 static aggregation; native live-ledger gate `4d7a495`); BLOCKED/NO-RELEASE (current and configured live profiles) | `uv run --isolated python scripts/check_release_ledger.py --json` reports 55/55 static checks passing, including the bounded multipart, resource-profile, provider-outage, and credentials-manager children, two pending worker security findings-review evidence items, and `NO-RELEASE`. The unconfigured 77-check `--live --json` snapshot records 58 pass/19 blocked/0 fail with four pending items and is retained at [`provenance/release_ledger_live_current.json`](../../mas/docs/provenance/release_ledger_live_current.json); the refreshed configured Compose 77-check profile records 67 pass/10 blocked/0 fail with four pending items and is retained at [`provenance/release_ledger_live.json`](../../mas/docs/provenance/release_ledger_live.json). Native preflight, image identity, Firecracker/gVisor, provider/KMS host configuration, outbound mail, self-improvement source, and security-review children remain explicit; the independently retained corrected resource and credentials evidence is scalar-only and does not change `NO-RELEASE`; licence metadata remains non-gating. |
