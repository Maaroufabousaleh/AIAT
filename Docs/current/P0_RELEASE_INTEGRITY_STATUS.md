...
- The 2026-08-18 continuation verification re-runs the static release ledger
  ...
  regression. The selected-model host certificate also passes with the
...
| Gateway worker/mail-edge local composition | PASS (local scalar + durable dual-Postgres composition + signed and raw provider-facing ingress; external provider sources open) | Commits `6ebb12c`, `fa42284`, `67f1599`, and `0e0a76f`; the scalar fixture and `check_gateway_worker_mail_edge_postgres.py --json --identity-ingress` plus `--provider-ingress` pass with the real `GatewayWorkerAdapter`/`WorkerRunController`, exact fixture provider/model usage, normalized delivery/verified-webhook/bounce observations, signed and Resend/Svix raw-body replay/conflict/tamper checks, durable provider-message worker/trace correlation, independent worker/identity Postgres reopen, payload-free cross-store correlation, and scoped cleanup. The certificates make no external-provider callback, external delivery, selected live worker, recovery, sandbox, or host-runtime claim. Evidence is [`gateway_worker_mail_edge_fixture.json`](../../mas/docs/provenance/gateway_worker_mail_edge_fixture.json), [`gateway_worker_mail_edge_postgres_evidence.json`](../../mas/docs/provenance/gateway_worker_mail_edge_postgres_evidence.json), and [`gateway_worker_mail_edge_provider_postgres_evidence.json`](../../mas/docs/provenance/gateway_worker_mail_edge_provider_postgres_evidence.json); licence metadata remains separate/not-gated. |
...
| Explicit live worker-plane provider runner | PASS (implementation + fail-closed fixture boundary; live provider evidence not run) | Commit `f999695`; `check_gateway_worker_provider_live.py` and its focused tests pass targeted Ruff/pytest. The runner requires an exact selected model present in the configured gateway's read-only `/v1/models` listing and explicit `--allow-external-provider` opt-in before one bounded completion through the real host executor/controller/adapter chain. The default invocation exits `blocked`, generated text and credentials are excluded from reports, and no live result is claimed here; durable Postgres, independent hosts, gVisor/Firecracker, provider recovery, and mail-edge callback/bounce evidence remain open. Licence metadata remains non-gating. Evidence is the checked-in runner/test contract, not a live-provider certificate. |
