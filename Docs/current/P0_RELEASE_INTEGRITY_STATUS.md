...
| Gateway worker/mail-edge local composition | PASS (local scalar + durable dual-Postgres composition + signed delegated ingress; raw provider sources open) | Commits `6ebb12c`, `fa42284`, and `67f1599`; the scalar fixture and `check_gateway_worker_mail_edge_postgres.py --json --identity-ingress` pass with the real `GatewayWorkerAdapter`/`WorkerRunController`, exact fixture provider/model usage, normalized delivery/verified-webhook/bounce observations, signed route replay/conflict/tamper checks, independent worker/identity Postgres reopen, payload-free cross-store correlation, and scoped cleanup. The certificate makes no raw external-provider callback, external delivery, selected live worker, recovery, sandbox, or host-runtime claim. Evidence is [`gateway_worker_mail_edge_fixture.json`](../../mas/docs/provenance/gateway_worker_mail_edge_fixture.json) and [`gateway_worker_mail_edge_postgres_evidence.json`](../../mas/docs/provenance/gateway_worker_mail_edge_postgres_evidence.json); licence metadata remains separate/not-gated. |
