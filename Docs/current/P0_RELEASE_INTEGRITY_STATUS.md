...
`d8ab39b` harden one-shot certification authorization, run-scoped MCP
...
- The latest unconfigured live aggregate (`57504a7`, observed
  `2026-08-19T04:47:35Z`) is retained at
  [`provenance/release_ledger_live_current.json`](../../mas/docs/provenance/release_ledger_live_current.json):
  71 pass, 0 fail, 14 externally blocked, and four pending out of 85. The
  resource-profile, multipart, provider-process outage, encrypted-backup,
  fresh-process restore, and AIAT credentials boundary children pass; native
  host/runsc, image refs, trace/runtime/model, gVisor/Firecracker, mail, and
  self-improvement prerequisites remain blocked.
  The configured retained profile remains 76 pass/0 fail/5 blocked across 81.
- The host-safe local Compose sweep (`38a8ab7`, evidence refresh `5f0c611`, observed
  `2026-08-21T17:42:45Z`) uses published loopback endpoints through
  `check_release_ledger.py --live --compose-local --json` and records 79
  pass, 0 fail, and 6 blocked across 85 checks with four pending evidence
  items. Its scalar summary is retained at
  [`provenance/release_ledger_live_compose_local_current.json`](../../mas/docs/provenance/release_ledger_live_compose_local_current.json).
  That historical snapshot's blockers were native release environment/image
  identity, gVisor, Firecracker, outbound mail, and self-improvement source
  selection; the default Cloudflare/Resend email identity gate was subsequently
  closed by the live certification recorded in
  [`../../Docs/AIAT_Email_Identity_Live_Certification.md`](../../Docs/AIAT_Email_Identity_Live_Certification.md).
  This profile does not change `NO-RELEASE`. A transient first aggregate
  `network_boundary:live` health failure was excluded after the narrow rerun
  and broader rerun both passed; it is classified as infrastructure/environment
  timing, not a provider or code failure.
- The documentation-index verifier fix (`e4a6dbf`) removes WSL/DrvFS symlink
  ...
  59/59 pass with `NO-RELEASE`. The release inventory follow-up (`aa7d2b2`)
  registers the bounded object-store lifecycle and trace-retention execution
  checks; `57504a7` adds encrypted-backup and clean-process restore checks.
  The current scalar summary is retained at
  [`provenance/release_ledger_static_current.json`](../../mas/docs/provenance/release_ledger_static_current.json).
  This is a validation-coverage change only and
...
- The 2026-08-19 pre-certification ordered gate sweep reruns the native preflight, AIAT
  ...
  contracts and focused regression suites pass; that historical snapshot's
  live blockers remain confined
  to the documented operator-owned host, runtime, artifact, credential, or
  signal-selection prerequisites. The final static aggregation remains 59/59
  pass with `NO-RELEASE`.
- The 2026-08-18 continuation verification re-runs the static release ledger
  at 59/59 pass and confirms the API/protocol contract, documentation index,
...
- The supported repository regression suite is green after `dc8a19e` repaired
  ...
  137-model/271-operation generated API contract. The focused storage,
...
- The AIAT credentials-manager live certificate (`12ba7c7`, scalar evidence
  ...
  refreshed at 2026-08-21T17:49:10Z with evidence commit `4c6f400`; release
  child `credentials_manager_live` registered in `d101901`) passes
...
- The remaining live-gate probes were rerun after correcting the test harness
  before the later email certification: outbound mail was blocked only for
  operator-owned relay credentials and a safe recipient, while self-improvement
  candidate detection was blocked only for an operator-selected signal
  source/project scope. The default Cloudflare/Resend email identity path is
  now live-certified in [`../../Docs/AIAT_Email_Identity_Live_Certification.md`](../../Docs/AIAT_Email_Identity_Live_Certification.md);
  self-improvement still requires its operator-selected signal source. Their
  deterministic fixture checks and the corrected 56-test mail/self-improvement
  regression slice pass; the malformed identity-test collection command is
  excluded as a harness/configuration failure.
...
- The current unconfigured local 2026-08-19 81-check profile records 67
  ...
  release-host preflight is the `release_environment:live` child and reports
  WSL2, missing `runsc`, dirty worktree, and absent immutable image refs as
  safe blockers. The current summary is retained at
  [`provenance/release_ledger_live_current.json`](../../mas/docs/provenance/release_ledger_live_current.json).
...
- `scripts/generate_typescript_api.py` turns the 137 OpenAPI component schemas
...
- `scripts/generate_python_api.py` emits the matching 137 Python `TypedDict`
...
| Shared identity-resource route matrix | PASS (source-built Playwright; preparatory P1) | Commit `485dfd2`; `PLAYWRIGHT_BASE_URL=http://127.0.0.1:4145 npx playwright test identity-resource-matrix.spec.ts --workers=1 --reporter=line` passes 9/9 with safe metadata-only fixtures across identities, approvals, audit, sessions, external accounts, domains, relay, mailboxes, and outbound mail; the default email identity live certification is recorded separately |
...
| Sandbox runtime readiness | PASS (static contract + retained native Ubuntu gVisor certificate; local WSL probe blocked) | Commits `a24c554`, `2c098f5`, and retained workflow run `32541110299`; `uv run --isolated pytest packages/mas-core/tests/test_sandbox_runtime_readiness.py -q` and `uv run --isolated python scripts/check_sandbox_runtime_readiness.py --json` reconcile 39 manifests, 10 hardened external workers, and the OpenCode Compose runtime’s internal-only network, no host ports, non-root/read-only execution, cap-drop ALL, no-new-privileges, bounded CPU/memory/PIDs, and noexec/nosuid tmpfs. [`native_gvisor_certification_live.json`](../../mas/docs/provenance/native_gvisor_certification_live.json) records native Linux, `runsc` registration, digest-pinned smoke, sandbox, cleanup, and zero-residue PASS; the local WSL `--live --json` probe remains exit 2 because `runsc` is not registered there. No runc fallback is permitted; broader native-host, network-denial, canary, Firecracker, and upstream scan evidence remain separate |
...
| Machine-readable release ledger | PASS (64/64 static aggregation; native live-ledger gate `4d7a495`); BLOCKED/NO-RELEASE (current and configured live profiles) | `uv run --isolated python scripts/check_release_ledger.py --json` reports 64/64 static checks passing, including the checked-in migration-head guard alongside the bounded object-store lifecycle, OCI SSE/KMS contract fixture, fresh OpenCode candidate provenance/boundary contract, trace-retention execution checks, encrypted-backup and fresh-process restore checks, with `NO-RELEASE`; scalar summary is [`provenance/release_ledger_static_current.json`](../../mas/docs/provenance/release_ledger_static_current.json). The current authenticated local Compose sweep at 2026-09-15T12:11:27Z records 83 pass/6 blocked/0 fail across 89 checks with four pending items and is retained at [`provenance/release_ledger_live_compose_local_current.json`](../../mas/docs/provenance/release_ledger_live_compose_local_current.json). The migration-head live check is blocked because the host-side read-only DSN cannot reach the published port; the running local database remains at 0042 while source is 0045. Native preflight, image SBOM/scan artifacts, Firecracker/gVisor, provider/KMS target configuration, outbound mail, self-improvement source, and security-review children remain explicit; licence metadata remains non-gating. |
...
| Host-safe local Compose release-ledger sweep | BLOCKED/NO-RELEASE (83/89 pass, 6 blocked, 0 fail) | The latest authenticated sweep at `2026-09-15T12:11:27Z` against revision `0f068476db908ee43bbc52900bf421394a121e6c` uses `check_release_ledger.py --live --compose-local` with published loopback endpoints and process-provided operator/tool-service authentication; no credentials or payloads are retained. Its scalar summary is [`provenance/release_ledger_live_compose_local_current.json`](../../mas/docs/provenance/release_ledger_live_compose_local_current.json). Four pending evidence items remain. Current blockers are native-host/environment, database migration-head reachability, deployment-image provenance, Firecracker worker-pool readiness, outbound mail lifecycle, and operator-selected self-improvement scope; historical 79/85 wording above remains historical. |
