...
- The evidence record page now fetches a read-only `aiat.evidence-detail.v1` projection for project, flow, flow-instance, worker, worker-run, credential, dead-letter, runtime, trace, model, and integration references. The dashboard proxy selects matching records from the model catalogue/integration list, allow-lists backend paths and scalar fields, bounds strings/numbers, drops nested payloads/secrets/configuration/profile bindings/trace items, labels temporary unavailability without invalidating the citation, and keeps unsupported kinds identity-only. Focused source-built coverage passes 6/6 in [`dashboard-evidence-detail.spec.ts`](../../mas/apps/mas-dashboard/e2e/dashboard-evidence-detail.spec.ts), including a 503 recovery check that preserves the identity and canonical link.
...
- [x] Add bounded CEO citation/deep-link coverage across tools, models, workers,
  ...
  implemented for eleven kinds, including model/integration selections and
  scalar-only trace summaries; broader detail kinds, stale/offline states, and
  full golden-path coverage remain.
