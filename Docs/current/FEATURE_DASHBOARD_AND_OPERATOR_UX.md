...
- The evidence record page now fetches a read-only `aiat.evidence-detail.v1` projection for project, flow, flow-instance, worker, worker-run, credential, dead-letter, runtime, trace, model, and integration references. The dashboard proxy selects matching records from the model catalogue/integration list, allow-lists backend paths and scalar fields, bounds strings/numbers, drops nested payloads/secrets/configuration/profile bindings/trace items, labels temporary unavailability without invalidating the citation, and keeps unsupported kinds identity-only. Focused source-built coverage passes 5/5 in [`dashboard-evidence-detail.spec.ts`](../../mas/apps/mas-dashboard/e2e/dashboard-evidence-detail.spec.ts).
