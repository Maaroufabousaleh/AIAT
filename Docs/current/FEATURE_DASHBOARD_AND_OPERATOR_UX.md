...
The Project Evidence package now applies the same canonical-read boundary:
401/403 responses expose a named access-denied region, retain a previously
loaded package as read-only context, hide Refresh/Retry and package controls,
and preserve safe Back to project navigation. Its source-built stale,
initial-denial, and retained-read denial matrix passes 3/3 (`00f81b5`).
...
CEO/owning-section navigation. The source-built scalar, stale, initial-denial,
...
Project Detail now also fails closed when the canonical project read or a
workflow/mutation response returns 401/403: it exposes a named access-denied
region, retains only the last-known project header as read-only context, clears
pending interaction state, and hides refresh, retry, tabs, panels, and all
workspace, workflow, flow, context, evidence, repository, approval, transition,
and other mutation controls. The source-built initial-denial,
retained-header-denial, workflow-mutation-denial, and transient-recovery
matrix passes 4/4 (`0671eaa`).
...
- The real JWT/bcrypt dashboard authentication module has a Node-level harness at [`scripts/test-auth.mjs`](../../mas/apps/mas-dashboard/scripts/test-auth.mjs), run with `npm run test:auth`; it verifies fail-closed missing-hash behavior, bcrypt success/rejection, signed-token validation, tamper rejection, and expiry rejection without requiring a live dashboard server. API-route proxy and browser behavior remain covered by the Playwright suite.
...
- The Project evidence page reads the canonical package with `cache: "no-store"`, retains the last successful package when a refresh fails, labels the page as showing last-known evidence, and exposes a keyboard-visible Retry action; the first-load failure remains explicit. Its named main/package sections, labeled back/refresh actions with 44px targets, and captioned/scoped evidence table form a focused accessibility baseline. A 401/403 package response now exposes a named access-denied region, preserves a previously loaded package as read-only context, hides Refresh/Retry and package controls, and retains safe Back to project navigation. [`projects/[id]/evidence/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/projects/[id]/evidence/page.tsx>) and [`project-evidence-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/project-evidence-states.spec.ts) cover failed refresh retention, successful recovery, initial denial, retained-read denial, and these semantics 3/3 against the source-built dashboard (`bc80ad5`, `89091c1`, `00f81b5`). Full project-page composition and live provider/worker evidence remain open.
...
- The new-flow builder now fails closed when its governed worker/profile or canonical template reads, create mutation, or readiness dry-run returns 401/403. It exposes a named access-status region, keeps the current draft canvas visible as read-only context, hides template application, the node palette, node configuration, validation, activation, and creation controls, and preserves the Back to flows link. [`flows/new/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/flows/new/page.tsx>) and [`flow-builder-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/flow-builder-states.spec.ts) cover initial catalogue denial, create denial, and readiness-validation denial 3/3 (`b07299b`).
...
- Project Detail now fails closed on canonical project-read and workflow/mutation 401/403 responses: it exposes a named denial region, retains only the last-known project header as read-only context, clears pending interaction state, and hides refresh, retry, tabs, panels, and workspace/workflow/flow/context/evidence/repository/approval/transition mutations. The source-built initial-denial, retained-header-denial, workflow-mutation-denial, and transient-recovery matrix passes 4/4 (`0671eaa`).
...
- The CEO Command Center chat fetches bounded history with `cache: "no-store"`, guards obsolete history/live callbacks, keeps history visible after a stream failure, labels the transcript as last known, and exposes a keyboard-visible Retry action. A 401/403 history, SSE, or message-submission response now exposes a named access-status region, retains any loaded transcript as read-only context, invalidates the shared stream hook's in-flight callbacks, and hides Clear/retry, quick commands, composer, and confirmation controls. [`ceo/chat/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/ceo/chat/page.tsx>), [`use-ceo-stream.ts`](../../mas/apps/mas-dashboard/lib/ceo-feed/use-ceo-stream.ts), and [`ceo-chat-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/ceo-chat-states.spec.ts) cover stale recovery, history denial, message denial, and the existing accessibility baseline 3/3 (`beabb95`, `8ffb5df`, `038d5f2`). Native/live Redis/router and full WCAG evidence remain open.
...
- Project-detail first-load/retry and canonical-read/workflow-mutation access-denied state: [`projects/[id]/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/projects/[id]/page.tsx>), [`project-detail-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/project-detail-states.spec.ts) (`0671eaa`)
- Project evidence package stale/retry and canonical-read access-denied state: [`projects/[id]/evidence/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/projects/[id]/evidence/page.tsx>), [`project-evidence-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/project-evidence-states.spec.ts) (`00f81b5`)
...
- Evidence-detail accessibility and bounded scalar-read access-denied state: [`evidence/[kind]/[id]/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/evidence/[kind]/[id]/page.tsx>), [`dashboard-evidence-detail.spec.ts`](../../mas/apps/mas-dashboard/e2e/dashboard-evidence-detail.spec.ts) (`32f3a76`, `23e2db9`)
...
- CEO Command Center chat stale/retry, accessibility, and access-denied state: [`ceo/chat/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/ceo/chat/page.tsx>), [`use-ceo-stream.ts`](../../mas/apps/mas-dashboard/lib/ceo-feed/use-ceo-stream.ts), [`ceo-chat-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/ceo-chat-states.spec.ts) (`beabb95`, `8ffb5df`, `038d5f2`)
