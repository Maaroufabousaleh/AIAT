...
- The Project evidence page reads the canonical package with `cache: "no-store"`, retains the last successful package when a refresh fails, labels the page as showing last-known evidence, and exposes a keyboard-visible Retry action; the first-load failure remains explicit. [`projects/[id]/evidence/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/projects/[id]/evidence/page.tsx>) and [`project-evidence-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/project-evidence-states.spec.ts) cover failed refresh retention and successful recovery 1/1 against the source-built dashboard (`bc80ad5`). Full project-page composition and live provider/worker evidence remain open.
...
- Flow editor load/stale/retry state: [`flows/[id]/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/flows/[id]/page.tsx>), [`flow-store.ts`](../../mas/apps/mas-dashboard/lib/flow-store.ts), [`flow-editor-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/flow-editor-states.spec.ts)
...
- Improve stale/conflict/rollback and partial-evidence experiences; system
  ...
  and flow editor now have explicit
