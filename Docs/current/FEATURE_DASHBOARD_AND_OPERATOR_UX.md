...
- The evidence record page now fetches a read-only `aiat.evidence-detail.v1` projection for project, flow, flow-instance, worker, worker-run, credential, dead-letter, runtime, and trace references. The dashboard proxy allow-lists backend paths and scalar fields, drops nested payloads/secrets and trace items, labels temporary unavailability without invalidating the citation, and keeps unsupported kinds identity-only. Focused source-built coverage passes 3/3 in [`dashboard-evidence-detail.spec.ts`](../../mas/apps/mas-dashboard/e2e/dashboard-evidence-detail.spec.ts).
...
- [x] Add bounded CEO citation/deep-link coverage across tools, models, workers,
  ...
  implemented for nine kinds, including scalar-only trace summaries; broader
  detail kinds, stale/offline states, and full golden-path coverage remain.
