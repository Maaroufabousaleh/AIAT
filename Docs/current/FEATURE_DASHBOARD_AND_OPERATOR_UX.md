...
- Improve stale/conflict/rollback and partial-evidence experiences; system
  visualization, identity tables, PM integrations, project detail, the combined
  governance read surface, and System Control now have explicit stale/partial/
  conflict and first-load/offline retry states, while broader page-specific denial
  and rollback coverage remains.
