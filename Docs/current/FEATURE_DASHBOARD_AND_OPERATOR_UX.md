...
- The evidence record page now fetches a read-only `aiat.evidence-detail.v1` projection for project, flow, flow-instance, worker, worker-run, credential, dead-letter, and runtime references. The dashboard proxy allow-lists backend paths and scalar fields, drops nested payloads/secrets, labels temporary unavailability without invalidating the citation, and keeps unsupported kinds identity-only. Focused source-built coverage passes 2/2 in [`dashboard-evidence-detail.spec.ts`](../../mas/apps/mas-dashboard/e2e/dashboard-evidence-detail.spec.ts).
...
- [x] Add bounded CEO citation/deep-link coverage across tools, models, workers,
  ...
  evidence route never renders payloads. The eight-kind scalar detail projection
  is now implemented; broader detail kinds, stale/offline states, and full
