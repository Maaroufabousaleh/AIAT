...
- The flow editor reads a route-selected flow through the canonical store, shows an explicit first-load unavailable state with Retry instead of exposing a blank editable canvas, and provides Refresh plus retained last-known flow/Retry recovery after a failed refresh. Backend error details remain visible, and source-built `flow-editor-states.spec.ts` covers first-load failure, retained nodes, stale labeling, and successful recovery 1/1 (`b5098e7`).
...
- Flows list stale/retry recovery: [`page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/flows/page.tsx>) and [`flows-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/flows-states.spec.ts) (commit `a0faf5b`, source-built 1/1)
- Flow editor load/stale/retry recovery: [`flows/[id]/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/flows/[id]/page.tsx>), [`flow-store.ts`](../../mas/apps/mas-dashboard/lib/flow-store.ts), and [`flow-editor-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/flow-editor-states.spec.ts) (commit `b5098e7`, source-built 1/1)
