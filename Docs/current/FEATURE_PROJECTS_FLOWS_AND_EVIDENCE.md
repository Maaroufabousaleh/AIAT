...
The project workspace sub-surface now also retains its last successful summary through failed workspace/repository refreshes and recovers through Retry (`cb1c665`, source-built `project-workspace-states.spec.ts` 1/1); this is bounded dashboard recovery evidence, not live worker/provider certification.
...
- The project workspace sub-surface retains the last successful workspace summary when `/workspace` or `/repository` refreshes fail, preserves the last repository snapshot across a partial failure, labels retained data, and exposes Retry for stale and first-load failures. [`project-workspace-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/project-workspace-states.spec.ts) covers the source-built failure → retained workspace → recovered workspace path 1/1 (`cb1c665`).
...
- Project workspace stale/retry recovery: [`projects/[id]/page.tsx`](<../../mas/apps/mas-dashboard/app/(dashboard)/projects/[id]/page.tsx>) and [`project-workspace-states.spec.ts`](../../mas/apps/mas-dashboard/e2e/project-workspace-states.spec.ts) (commit `cb1c665`, source-built 1/1)
