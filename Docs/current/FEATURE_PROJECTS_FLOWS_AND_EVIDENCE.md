...
**Baseline:** 2026-08-10
**Status:** broad implementation present; versioned node-schema/evidence contracts, generated editable node forms, explicit graph-rewrite migration, operator-approved saved-definition worker migration, consolidated project evidence packages, and authenticated canonical self-improvement project API/storage/reference/action paths are implemented, while live worker/recovery remains. The deterministic evidence-package core and policy-scope resolver are reviewed and committed in `a44a1aa`, with package-level workflow exports isolated in `d0472af`; API/storage/dashboard snapshot wiring remains a separate reviewed implementation group.
...
- Flow definitions expose deterministic export/hash, node/edge diff, import, publish, and deprecate endpoints; compatible instance migration verifies schema/version and active-node identity/type, while explicitly opted-in graph rewrites require a one-to-one active-node mapping, preserve execution history, and record the mapping in migration evidence. The dashboard now proxies the explicit saved-definition worker migration through its operator boundary.
- Flow definitions expose deterministic export/hash, node/edge diff, import, publish, and deprecate endpoints; existing instances continue to pin their flow version and terminal instances remain immutable.
...
- Evidence-package core review batch: commits `a44a1aa` and `d0472af`; [`test_evidence_package_runner.py`](../../mas/packages/mas-core/tests/test_evidence_package_runner.py) and [`check_project_evidence_package.py`](../../mas/scripts/check_project_evidence_package.py) cover deterministic grouping, metadata-only notices, and fail-closed live mode, with package-level workflow exports kept self-contained.
...
- Extend graph-rewrite migration only with separately reviewed mapping semantics; the current path is intentionally limited to active nodes, one-to-one mappings, unchanged node types, and preserved history.
