...
| --- | --- | --- |
| Source revision at refresh | **`fe2b768e`** | The broad snapshot below remains anchored to this implementation/test baseline; the later OpenCode/OpenHands benchmark-corpus slice has its own focused validation row and is not being retroactively represented by this older snapshot. These checks do not close live/operator gates. |
...
| Clean-clone static release certificate | **PASS (2026-09-19)** | A fresh local clone of `76272905db777829ccf21b61748eb467cafaf645` has zero changed paths and reproduces 64/64 static checks with two pending evidence items and `NO-RELEASE`; the scalar certificate is [`release_ledger_clean_candidate_static.json`](../../mas/docs/provenance/release_ledger_clean_candidate_static.json). This does not close live, provider, native-host, security-review, or activation gates. |
