...
external provider-backed model execution, sandbox certification, and full
worker certification remain separate. `f999695` adds an explicit opt-in
worker-plane provider runner; its default invocation is fail-closed and no live
provider run is claimed until an operator supplies a selected model, gateway,
credentials, and the opt-in flag.
...
- `scripts/check_gateway_worker_provider_live.py` (`f999695`) is the explicit
  ...
  text or credentials. The default invocation is blocked and the current
  repository has no live-provider evidence from this runner; durable Postgres,
  independent hosts, gVisor/Firecracker, provider recovery, and mail-edge
  callback/bounce read-back remain separate gates.
