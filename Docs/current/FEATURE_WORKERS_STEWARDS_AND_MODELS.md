...
- Runtime benchmark probe (`ad31793`): [`mas/scripts/check_runtime_benchmarks.py`](../../mas/scripts/check_runtime_benchmarks.py) and [`test_runtime_benchmarks.py`](../../mas/packages/mas-core/tests/test_runtime_benchmarks.py). It is read-only and reports explicit `blocked` status when the API, package, or validation path is unavailable; a dependency dry-run does not certify a worker canary or rollback.
