...
- Code review (`fc528a8`, implementation hardening `5b830e9`) now has a reproducible AIAT deterministic diff reviewer as its
  ...
  exact source/revision/version and representative review evidence. Commit
  `10b1af2` also bounds the optional external command boundary: empty,
  malformed, or non-object successful stdout becomes a stable degraded result
  instead of a generic registry error, without retaining an untrusted review
  payload as evidence.
- Security scanner aliases (`fc528a8`, implementation hardening `5b830e9`) now have a deterministic contract fixture: the
  ...
  sandbox result. Commit `7ff464d` also normalizes malformed, empty, or
  non-object responses from the configured sandbox adapter into scalar
  degraded results without retaining raw adapter output. Commit `31a4cd2`
  extends the same boundary to direct gVisor runtime probes and container
  launches: timeout, Docker/metadata failure, invalid runtime response, and
  launch failure are stable infrastructure reasons without raw exception text;
  a missing `runsc` runtime remains fail-closed. Commit `f04a71a` also scrubs
  the runner's top-level malformed-request, subprocess, and unexpected-error
  paths into scalar reason codes without an `error` field.
...
- Sandbox runtime readiness probe (`a24c554`): [`mas/scripts/check_sandbox_runtime_readiness.py`](../../mas/scripts/check_sandbox_runtime_readiness.py) and [`test_sandbox_runtime_readiness.py`](../../mas/packages/mas-core/tests/test_sandbox_runtime_readiness.py). Static reconciliation passes all 39 manifests with 10 hardened external workers; the local WSL Docker host reports `runsc` unavailable and therefore remains blocked without a `runc` fallback. The separate retained native Ubuntu certificate [`native_gvisor_certification_live.json`](../../mas/docs/provenance/native_gvisor_certification_live.json) from workflow run `32541110299` passes `runsc` registration, smoke, sandbox, cleanup, and zero-residue checks; broader native-host and network-denial evidence remain separate.
...
- Security adapter aliases/fixture and bounded scanner/sandbox output (`fc528a8`, `583796f`, `7e41ea6`, `7ff464d`, `31a4cd2`, `f04a71a`): [`mas/scripts/check_security_adapters.py`](../../mas/scripts/check_security_adapters.py), [`SecurityScanTool`](../../mas/apps/tool-service/tool_service/tools/adapters.py), [`ToolRegistry`](../../mas/apps/tool-service/tool_service/registry.py), [`test_security_scan_reports_invalid_semgrep_shape_without_raising`](../../mas/apps/tool-service/tests/test_default_shipped_tool_catalog.py), [`test_security_scan_reports_degraded_skillspector_output`](../../mas/apps/tool-service/tests/test_default_shipped_tool_catalog.py), [`test_command_run_safe_reports_degraded_sandbox_output`](../../mas/apps/tool-service/tests/test_default_shipped_tool_catalog.py), [`test_sandbox_runner_classifies_runtime_probe_failures_without_raw_errors`](../../mas/apps/tool-service/tests/test_real_adapter_backends.py), and [`test_sandbox_runner_cli_does_not_emit_raw_request_errors`](../../mas/apps/tool-service/tests/test_real_adapter_backends.py)
...
- [x] Export and reconcile the deterministic runtime/profile catalogue, add the bounded executive reconciliation report, `aiat.executive-views.v1` role projections, dedicated read-only `/executive/views/{role}` endpoints, role-scoped `aiat.executive-action.v1` CFO/CTO/CEO writes, and audit reservation/settlement invariants; retained local live evidence observes complete approved persisted coverage (93/93) after exact unreferenced smoke-fixture cleanup, with the explicit OpenCode/LiteLLM identity alias reconciled and no catalogue finding. The active registry now has 92 entries after retired Groq model removal; provider-specific live failover/recovery, broader governance forms, and broader budget-settlement chaos tests remain.
