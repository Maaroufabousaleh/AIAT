...
- `scripts/generate_worker_certification_matrix.py` generates the deterministic
...
- Certification now persists a compatibility-matrix row through
  ...
  status to certification evidence. This does not replace live certification.
...
- Default worker implementation binding matrix: [`mas/scripts/check_default_worker_bindings.py`](../../mas/scripts/check_default_worker_bindings.py) and [`test_default_worker_bindings.py`](../../mas/packages/mas-core/tests/test_default_worker_bindings.py). The static contract covers all 15 documented default slots, verifies each declared transport/isolation pair against `RUNTIME_CATALOG`, and requires matching runtime/integration adapter entrypoints; `--live` remains an explicit operator/environment boundary and never mutates runtime state.
- Worker-run lifecycle fixture: [`mas/scripts/check_worker_run_lifecycle.py`](../../mas/scripts/check_worker_run_lifecycle.py) and [`test_worker_run_lifecycle.py`](../../mas/packages/mas-core/tests/test_worker_run_lifecycle.py). The static fixture checks real controller ordering, failure normalization, and recovery invariants; `--live` reports the explicit operator/database boundary.
- Runtime readiness probe: [`mas/scripts/check_worker_runtime_readiness.py`](../../mas/scripts/check_worker_runtime_readiness.py)
- Runtime install-profile contract: [`mas/scripts/check_runtime_install_profile.py`](../../mas/scripts/check_runtime_install_profile.py)
- Sandbox runtime readiness probe: [`mas/scripts/check_sandbox_runtime_readiness.py`](../../mas/scripts/check_sandbox_runtime_readiness.py)
- Runtime benchmark probe: [`mas/scripts/check_runtime_benchmarks.py`](../../mas/scripts/check_runtime_benchmarks.py)
- Runtime adapter conformance probe: [`mas/scripts/check_runtime_adapter_conformance.py`](../../mas/scripts/check_runtime_adapter_conformance.py) and [`runtime_adapter_conformance_live.json`](../../mas/docs/provenance/runtime_adapter_conformance_live.json)
...
- Steward lifecycle contract: [`mas/scripts/check_worker_steward_contract.py`](../../mas/scripts/check_worker_steward_contract.py)
