...
The selected model-resolution certificate [`worker_host_model_resolution_postgres_evidence.json`](mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json) (`6cef1b8`) closes the local control-plane model edge: an approved Model Profile/version is resolved deterministically, persisted as a snapshot, carried through the committed worker-host request/run, and matched to exact provider/model usage after Postgres reopen. The provider and model are local fixture identifiers; external provider execution, independent hosts, hardened sandbox operation, and provider-backed recovery remain separate gates.
