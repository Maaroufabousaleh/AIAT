...
| Hierarchy communication-policy visualization | **Implemented; E2E/live image evidence pending** | `HierarchyViz` exposes a sender-role overlay that labels and color-codes allowed/denied team paths from the same permission matrix used by policy (`8b7d9f1`). Dashboard typecheck and focused lint/build pass; the source-built E2E spec is checked in but has not yet executed successfully against a current image. The running Compose image predates the commit and its Docker context currently contains unreadable `.tmp-*` paths. |
...
- [x] Add the hierarchy communication-policy overlay (`8b7d9f1`) with
  ...
  typecheck/lint/build pass and E2E spec coverage is checked in (execution
  remains pending), while live Compose image
  evidence remains open because the deployed image predates the commit.
...
12. **Implemented communication-policy identity boundary (`fb39128`):** policy and message-router tests reject non-CEO envelopes whose declared sender role/team pair is incoherent, including direct worker-to-CEO/admin spoof paths, before Redis dedupe/enqueue. The hierarchy overlay (`8b7d9f1`) exposes the same allowed/denied paths to operators; live external-router, live dashboard, and image-refresh evidence remain separate.
...
4. Mobile/accessibility/theme completion and the remaining dashboard stale/offline/conflict states. Identity, PM integration, project-list/detail, system-visualisation, evidence-detail, governance, System Control, Tools catalogue, dead-letter queue, credentials, Metrics, Flows, Container Logs, Agent Streams, Hiring Board, and CEO Live Feed read surfaces now have focused stale/retry coverage; native-Linux/page-level visual and broader recovery evidence remain open.
...
- `mas/apps/mas-dashboard/components/system-viz/HierarchyViz.tsx`, the system
  ...
  checks pass while live image evidence remains pending.
