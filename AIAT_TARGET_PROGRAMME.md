...
The deterministic placement certificate [`worker_placement_contract.json`](mas/docs/provenance/worker_placement_contract.json) adds the AIAT-owned `aiat.worker-placement.v1` predicate over explicit host snapshots: readiness/lease, labels, capabilities, sandbox/isolation, capacity, deterministic ordering, and duplicate-ID rejection are checked without mutation. Live multi-host scheduling, host-loss/split-brain, and Firecracker evidence remain open.
The durable host certificate [`worker_host_registry_postgres_evidence.json`](mas/docs/provenance/worker_host_registry_postgres_evidence.json) proves AIAT-owned host registration with token-digest authentication, wrong-token rejection, heartbeat lease renewal, redacted host/public placement snapshots, connection-reopen read-back, and expired-lease visibility at migration `0037_worker_host_registry`. Durable host recovery, live multi-host scheduling, host-loss/split-brain avoidance, and Firecracker certification remain open.
The reservation certificate [`worker_host_reservations_postgres_evidence.json`](mas/docs/provenance/worker_host_reservations_postgres_evidence.json) proves host-lease/readiness enforcement, row-locked capacity limits, idempotent reservation keys, commit/release/expiry transitions, scalar capacity read-back, and cleanup at migration `0038_worker_host_reservations`. It does not claim live multi-host selection, scheduler dispatch, host-loss/split-brain recovery, or Firecracker certification.
