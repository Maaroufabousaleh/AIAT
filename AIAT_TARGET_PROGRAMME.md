...
Pause and recovery use explicit `PAUSING`, `PAUSED`, and `RESUMING` transitions. Queue claims and heartbeats are leased. Transitions use compare-and-set semantics. Terminal results become authoritative only after artifacts and usage evidence are durably queryable. The bounded local certificate [`worker_lease_recovery_postgres_evidence.json`](mas/docs/provenance/worker_lease_recovery_postgres_evidence.json) proves the existing Postgres lease APIs reject a competing owner, bind heartbeats to the claimant, requeue one explicitly expired fixture lease, allow a second claimant to reclaim it, deny terminal re-claim, and preserve eight transitions through connection reopen. It does not claim a canonical host registry, placement scheduler, real host-loss/split-brain proof, or gVisor/Firecracker host certification. The refreshed certificate [`worker_version_pinning_postgres_evidence.json`](mas/docs/provenance/worker_version_pinning_postgres_evidence.json) at migration `0040_worker_run_skill_bundle_pin` proves that a running version-one worker run retains its shell, adapter, skill bundle, and steward references while the registry advances to version two and a new queued run uses the complete replacement set. Live dispatch, host/provider recovery, multi-host execution, and gVisor/Firecracker remain separate gates.
