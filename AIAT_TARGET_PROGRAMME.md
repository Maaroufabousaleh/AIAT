...
5. **Implemented contract:** production Compose infrastructure refs and Dockerfile bases are digest-pinned; resolve application image refs and align lockfiles, manifests, operational provenance, SBOMs, and notices.
6. **Implemented contract:** CEO service identity and persisted section ACLs with human/CEO/service/worker allow-deny API tests; run native deployment evidence.
...
10. **Implemented preparatory contract export:** `schemas/http/orchestrator.openapi.json` and the checked-in `aiat.v1` protocol schema are verified against runtime hashes by `scripts/check_api_contract.py`; deterministic dashboard TypeScript and internal Python SDK models/operation metadata are generated by the paired generators; external-language SDKs and modular router extraction remain P1 work after the P0 exit.
...
4. Mobile/accessibility/theme completion and dashboard stale/offline/conflict states.
