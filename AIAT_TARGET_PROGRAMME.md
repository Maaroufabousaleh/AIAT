...
The deterministic placement certificate [`worker_placement_contract.json`](mas/docs/provenance/worker_placement_contract.json) adds the AIAT-owned `aiat.worker-placement.v1` predicate over explicit host snapshots: readiness/lease, labels, capabilities, sandbox/isolation, capacity, deterministic ordering, and duplicate-ID rejection are checked without mutation. The scheduler certificate [`worker_host_scheduler_postgres_evidence.json`](mas/docs/provenance/worker_host_scheduler_postgres_evidence.json) proves deterministic multi-host selection, row-locked fallback to a lower-ranked host, idempotent replay, blocked full-capacity outcomes, connection-reopen read-back, and scoped cleanup at migration `0038_worker_host_reservations`; live worker dispatch and Firecracker evidence remain open.
The durable host certificate [`worker_host_registry_postgres_evidence.json`](mas/docs/provenance/worker_host_registry_postgres_evidence.json) proves AIAT-owned host registration with token-digest authentication, wrong-token rejection, heartbeat lease renewal, redacted host/public placement snapshots, connection-reopen read-back, and expired-lease visibility at migration `0037_worker_host_registry`. The current fencing migration `0039_worker_host_fencing` adds an explicit host lease generation to public projections and placement snapshots.
