...
phase on the current `origin/main` implementation/content baseline `2645309f`
(later commits are documentation-only). This phase expands
...
| Universal worker contract | **Implemented foundation** | `aiat.worker.v1`, protocol negotiation, adapter API versioning, normalized events/results, run lifecycle, checkpoints, artifacts, usage, pause/resume/cancel, queue leases, and recovery APIs exist. All 39 team-runner agents now declare exact `worker_manifest_ref` identities; static reconciliation and production startup checks fail closed on missing/mismatched bindings while remaining read-only. The deterministic `check_worker_run_lifecycle.py` fixture proves controller ordering/requeue invariants, `acd3f06` adds a real local Postgres controller/native-adapter run with connection-reopen, payload-free trace, and scoped-cleanup evidence, `a413997` adds a local Postgres lease certificate for claim exclusivity, owner-bound heartbeats, expiry/requeue, reclaim, and durable transition read-back, `73c0bda` adds committed worker-plane host admission/claim/release through `aiat.worker-host-execution.v1`, and `6cef1b8` plus `9a7db70` add local approved Model Profile resolution/snapshot propagation and pre-claim snapshot consistency through that host edge. `199eb5b` now rejects governed result provider/model usage that does not match the immutable resolution snapshot before usage or terminal evidence is persisted. `080ee18` adds the bounded `aiat_gateway` model-worker adapter and fixture checker for prompt normalization and exact usage attribution without external calls; `f6baebc` registers that transport in the worker manifest contract, builtin runtime catalogue, and read-only reconciliation checker; `cec1e4c` starts/stops an owned gateway client and rejects oversized prompt/message input and non-finite temperatures; `cbbfe56` certifies the real AIAT gateway-client HTTP boundary and retry path through a loopback transport; `6ebb12c` composes the real gateway adapter/controller with scalar worker and mail-edge evaluators in a payload-free local fixture; `b2ae516` classifies gateway input, transient, and permanent failures without leaking provider response text; `38c99f4` certifies the same gateway worker through the real host-executor/controller boundary over a bounded in-memory worker-plane fixture, including admission, claim, exact usage, terminal settlement, release, and payload-free scalar trace coverage; `2abc02a` certifies transient/permanent gateway failure settlement through that host edge with released bindings and secret-safe metadata. External provider-backed worker, deployed sandbox, and multi-host recovery evidence remains open. |
...
| Model governance | **Implemented foundation** | Versioned model profiles, deterministic policy intersection, override approvals, resolution snapshots, LiteLLM-compatible routes, usage/budget records, explicit transient-status classification, persisted model/provider cooldown failover state, deterministic `aiat.model-profile-catalogue.v1` API/dashboard reconciliation, a fail-closed `scripts/check_model_profile_catalogue.py --live --require-approved` verifier, an idempotent conflict-preserving bootstrap for the evidence-referenced `opencode-phase0b-coding` profile, all registered model identities, and the `omniroute-coding` alias, bounded `aiat.executive-reconciliation.v1` reporting, `aiat.executive-views.v1` CFO/CTO/CEO role projections, dedicated read-only `/executive/views/{role}` endpoints, a secret-safe `scripts/check_executive_reconciliation.py --live --json` verifier with optional finding-free enforcement, and role-scoped `aiat.executive-action.v1` CFO/CTO/CEO write routes exist. The typed governance action panel is implemented; the 2026-08-18 read-only local evidence observes complete 93 approved covered profile versions out of 93 persisted versions with no findings after exact unreferenced smoke-fixture cleanup; that snapshot is historical, and the active registry now has 92 entries after removing retired Groq `llama-3.3-70b-versatile` while retaining `openai/gpt-oss-120b`. The explicit `aiat/omniroute-coding` identity reconciled to canonical `litellm/omniroute-coding`, a nine-model `/v1/models` route with all five AIAT aliases, and no completion request, provider call, routing mutation, or activation decision remain route-only evidence. The repeatable `scripts/check_model_gateway_readiness.py --live` boundary and [`model_gateway_readiness_live.json`](mas/docs/provenance/model_gateway_readiness_live.json) record the same route without a completion, provider call, or mutation. The governed `aiat_gateway` worker adapter fixture (`080ee18`) now normalizes a resolved exact model into one controller run and usage record without external calls; `f6baebc` makes the transport discoverable through the manifest/runtime catalogue/reconciliation contract, `cec1e4c` hardens its owned-client lifecycle and input bounds, `cbbfe56` certifies the loopback HTTP client boundary/retry path, and `6ebb12c` joins its real controller result with scalar worker/mail-edge coverage without claiming live provider evidence; `b2ae516` makes transient/permanent/input failure semantics explicit and secret-safe; `38c99f4` carries that exact model/usage path through bounded host admission, claim, terminal settlement, and binding release with no external provider call; `2abc02a` proves transient/permanent gateway failure settlement through the same host edge with released bindings and secret-safe metadata. Provider-specific live recovery, broader governance forms, and immutable gateway pinning remain release work. |
...
| Identity and email control plane | **Implemented; production-certified for the default Cloudflare/Resend v1 path** | Dedicated identity service, Postgres store, signed clients, provider-neutral email identities and bindings, Cloudflare Worker/D1-only inbound sync with an optional Worker R2 profile, direct Resend API outbound, optional Stalwart full-mailbox support, approvals, external accounts, browser sessions, credential leases, sync/outbox, audit, and dashboard surfaces exist. The shared identity-resource dashboard surface now exposes named main/status/metadata/table regions, explicit busy state, decorative-icon suppression, 44px refresh/retry/action targets, and a distinct 401/403 access-denied region that removes misleading retry actions while preserving already loaded metadata-only rows (`a260e04`, `0974434`) alongside stale/retry retention (`46eccee`, `651ad11`). The credentials manager now applies the same denial boundary to redacted credential metadata, hiding read/mutation controls and retaining only previously loaded rows (`982c9c0`). The route matrix covers identities, approvals, audit, sessions, external accounts, domains, relay, mailboxes, and outbound mail 9/9 with safe metadata-only fixtures (`485dfd2`). The AIAT-owned credentials boundary now has a reproducible private Compose probe (`a946368`, `f9527b3`) through `mas-orchestrator-api-1`: all 12 checks pass, zero fixture rows remain after teardown, and provider-managed KMS remains a separate unconfigured gate. The versioned external-account action taxonomy now makes rotation/closure human gates and suspension safety revocation explicit; independent fixtures cover its five-action/category/fail-closed contract plus the real service's signup idempotency, one-use leases, rotation/closure/suspension revocation, and secret-safe output. The local Cloudflare/Resend provider fixtures prove approval pause, direct API idempotency, recipient authorization, offline catch-up, and secret-safe reporting. Default-path public-domain, provider, and mail-path certification is recorded in `Docs/AIAT_Email_Identity_Live_Certification.md`; optional Stalwart/R2 profiles, broader outage/restore evidence, and unrelated provider gates remain separate. |
...
| Long-term memory/workflow infrastructure | **Partial** | Postgres/pgvector context and checkpoint storage are real. The bounded, disabled-by-default Qdrant and Temporal adapter contracts now have exact-version, opaque-namespace, bounded-input, stable-failure, checksum/scalar-output boundaries (`0f1db19`); Letta remains adapter-backed. Letta, Qdrant, and Temporal external services are still not proven default runtime dependencies in the current stack, and live value/recovery evidence remains open. |
| Object storage | **Contract/copy/backup/migration, AIAT-owned encryption envelope, bounded scalar resource profile, and provider-process outage evidence implemented; production live work pending** | MinIO is the current S3-compatible artifact backend. The provider-neutral `aiat.object-store-conformance.v1` fixture, real-`BlobClient` `--live` conformance runner, checked-in private-network MinIO probe, bounded aggregate `--compose-local` release child, `aiat.object-store-copy.v1` verified-copy/parity helper, live source-inventory/target-parity runner, deterministic `aiat.object-store-backup.v1` manifest, clean-target `aiat.object-store-restore.v1` verifier, AIAT-owned `aiat.object-store-encrypted-backup.v1` AES-256-GCM envelope, encrypted ciphertext replication/read-back helper, fresh-process encrypted bundle restore certificate, three-provider backup/restore runner, `aiat.object-store-migration.v1` inventory/dual-write/cutover/rollback workflow, and `aiat.object-store-resource-profile.v1` scalar RSS/wall/CPU checker are implemented. The deployed local MinIO service has retained secret-safe 8/8 conformance and same-provider backup/restore reports at [`mas/docs/provenance/object_store_live_conformance.json`](mas/docs/provenance/object_store_live_conformance.json) and [`mas/docs/provenance/object_store_backup_restore_live.json`](mas/docs/provenance/object_store_backup_restore_live.json); the local encrypted envelope and fresh-process restore certificates are retained at [`mas/docs/provenance/object_store_encryption_evidence.json`](mas/docs/provenance/object_store_encryption_evidence.json) and [`mas/docs/provenance/object_store_clean_environment_restore_evidence.json`](mas/docs/provenance/object_store_clean_environment_restore_evidence.json). The corrected live migration rehearsal (`ecbef00`) and provider-diverse resource/multipart/outage waves (`3791b3f`/`c90fc13`, `a2f35de`, `d92b3dc`/`407a4f9`) across disposable MinIO/SeaweedFS pass identical checksum/read-back, cleanup, multipart abort, outage recovery, and scalar resource observations; the earlier nonexistent SeaweedFS alias run is explicitly excluded as harness/configuration invalid. The release ledger validates these scalar certificates through retained-live evidence paths. Provider-managed encryption/key custody, production routing/retention cutover, production resource budgets/portability, and clean-host/disaster-recovery remain open. |
...
historically record one operator-selected `llama-3.3-70b-versatile` completion
through the configured LiteLLM route, durable worker/identity Postgres reopen, delivered /
...
The default software company has 11 departments/offices and activates the 11 shells above. Its current starter budget is USD 100 with a maximum of 20 concurrent runs, requires human approval for external workers, and selects the canonical `sandboxed`/gVisor class as the default external-worker boundary; high-risk or gVisor-incompatible work requires `vm_isolated`/Kata.
...
Pause and recovery use explicit `PAUSING`, `PAUSED`, and `RESUMING` transitions. Queue claims and heartbeats are leased. Transitions use compare-and-set semantics. Terminal results become authoritative only after artifacts and usage evidence are durably queryable. The bounded local certificate [`worker_lease_recovery_postgres_evidence.json`](mas/docs/provenance/worker_lease_recovery_postgres_evidence.json) proves the existing Postgres lease APIs reject a competing owner, bind heartbeats to the claimant, requeue one explicitly expired fixture lease, allow a second claimant to reclaim it, deny terminal re-claim, and preserve eight transitions through connection reopen. It does not claim a canonical host registry, placement scheduler, real host-loss/split-brain proof, or gVisor/Firecracker host certification. The refreshed certificate [`worker_version_pinning_postgres_evidence.json`](mas/docs/provenance/worker_version_pinning_postgres_evidence.json) (`7c1ef74`, migration `0042_worker_run_host_binding`, extending `6a10b0e`) proves that a running version-one worker run retains its shell, adapter, skill bundle, steward identity, worker source/version metadata, and model-resolution snapshot while the registry advances to version two and a new queued run uses the replacement shell/adapter/bundle/model snapshot. Postgres reopen, payload-free output, and scoped cleanup pass. Live dispatch, provider/host recovery, multi-host process boundaries, and gVisor/Firecracker remain separate gates.
The deterministic placement certificate [`worker_placement_contract.json`](mas/docs/provenance/worker_placement_contract.json) adds the AIAT-owned `aiat.worker-placement.v1` predicate over explicit host snapshots: worker-plane identity, readiness/lease, labels, capabilities, sandbox/isolation, capacity, deterministic ordering, and duplicate-ID rejection are checked without mutation. Migration `0041_worker_host_planes` makes the worker plane explicit and rejects control/tool/data hosts with a `host_plane_mismatch` decision. The scheduler certificate [`worker_host_scheduler_postgres_evidence.json`](mas/docs/provenance/worker_host_scheduler_postgres_evidence.json) proves deterministic multi-host selection, row-locked fallback to a lower-ranked host, idempotent replay, blocked full-capacity outcomes, connection-reopen read-back, and scoped cleanup at migration `0038_worker_host_reservations`; live worker dispatch and Firecracker evidence remain open.
The run-host assignment certificate [`worker_run_host_binding_postgres_evidence.json`](mas/docs/provenance/worker_run_host_binding_postgres_evidence.json) at migration `0042_worker_run_host_binding` binds a durable Worker Run to the selected worker-plane reservation, preserves the host lease generation, enforces run/worker identity, replays assignment keys, settles owner-bound commit/release transitions, and survives connection reopen. It is assignment authority only; the committed-binding execution edge is certified separately.
The committed worker-plane host executor certificate [`worker_host_execution_postgres_evidence.json`](mas/docs/provenance/worker_host_execution_postgres_evidence.json) (`73c0bda`) now admits only a committed binding with matching host plane, lease generation, READY status, and current lease, claims the queued run, delegates to the canonical `WorkerRunController`/native fixture adapter, releases the binding, and verifies payload-free evidence after a Postgres reopen. This is local AIAT-owned native execution evidence; external provider-backed selected-model dispatch, deployed gVisor/Firecracker, external providers, remote runtimes, and provider-backed recovery remain separate gates.
The bounded host-composition certificate [`gateway_worker_host_fixture.json`](mas/docs/provenance/gateway_worker_host_fixture.json) (`38c99f4`) carries the governed gateway worker through the same host-executor boundary using committed in-memory binding/storage doubles. It proves worker-plane admission, queued claim, exact fixture model/usage attribution, terminal settlement, binding release, and payload-free scalar worker/model trace coverage. Its artifact is a synthetic report pointer without generated content; durable host storage, independent hosts, external providers, sandbox execution, and live recovery remain separate gates.
The durable host certificate [`worker_host_registry_postgres_evidence.json`](mas/docs/provenance/worker_host_registry_postgres_evidence.json) proves AIAT-owned host registration with token-digest authentication, wrong-token rejection, heartbeat lease renewal, redacted host/public placement snapshots, connection-reopen read-back, and expired-lease visibility at migration `0037_worker_host_registry`. Migration `0041_worker_host_planes` persists and reads back the host-plane identity, while the fencing migration `0039_worker_host_fencing` adds an explicit host lease generation to public projections and placement snapshots.
The reservation certificate [`worker_host_reservations_postgres_evidence.json`](mas/docs/provenance/worker_host_reservations_postgres_evidence.json) proves host-lease/readiness enforcement, row-locked capacity limits, idempotent reservation keys, commit/release/expiry transitions, scalar capacity read-back, and cleanup at migration `0038_worker_host_reservations`; current reservations also retain the host lease generation that created them.
The recovery certificate [`worker_host_recovery_postgres_evidence.json`](mas/docs/provenance/worker_host_recovery_postgres_evidence.json) proves split-brain fencing and host-loss recovery at migration `0039_worker_host_fencing`: re-registration advances the host generation and expires the prior incarnation's reservations, stale heartbeats are rejected, an expired READY lease is durably fenced to `OFFLINE`, active reservations from the lost generation are expired in the same transaction, placement excludes the recovered host, and connection-reopen read-back is durable. Live worker dispatch, gVisor/Firecracker, and provider-backed recovery remain separate evidence boundaries.

The concurrent native certificate [`worker_multi_host_execution_postgres_evidence.json`](mas/docs/provenance/worker_multi_host_execution_postgres_evidence.json) (`f9c717b`, extended by `d45e4dd`) extends the local host executor boundary to two distinct durable worker-host identities. Two committed reservations execute two queued runs concurrently, each preserves host lease-generation/current-lease equality, and both release with payload-free usage/artifact/trace evidence after a Postgres reopen and scoped cleanup. The extension races a duplicate claim for one run, requires one `worker_run_claim_failed` rejection, records exactly two adapter dispatches, and proves terminal and alternate-run-ID idempotency replays return the canonical run without redispatch. This is local fixture evidence only; independent deployed hosts, external provider-backed selected-model dispatch, gVisor/Firecracker, external providers, and host-loss/provider recovery remain separate gates.
The process-boundary certificate [`worker_independent_process_execution_postgres_evidence.json`](mas/docs/provenance/worker_independent_process_execution_postgres_evidence.json) (`cec6558`, corrected by `520c6bf`) launches two separate Python child processes on the same Compose host. Each process reconnects to Postgres and settles one committed binding through the production `WorkerHostExecutor`/`WorkerRunController`; the parent verifies distinct process IDs, two `SUCCEEDED` runs, two usage rows, two artifacts, three native spans per trace, payload-free coverage, independent reopen, and zero-row cleanup. This closes the local process-isolation prerequisite only, not independent deployed machines, host-loss/split-brain, provider, or sandbox evidence.
The fenced host-loss queue-recovery certificate [`worker_host_loss_queue_recovery_postgres_evidence.json`](mas/docs/provenance/worker_host_loss_queue_recovery_postgres_evidence.json) (`893293a`) adds the missing local recovery edge. A host-filtered reconciliation fences one expired worker host and reservation, the canonical Worker Run lease recovery requeues the expired claim, stale execution is rejected before dispatch, and the binding is reassigned to an alternate host for a native retry at attempt two. Postgres reopen, payload-free evidence, and scoped cleanup pass. Independent deployed-host recovery, gVisor/Firecracker, external providers, and provider-backed recovery remain separate gates.
The selected model-resolution certificate [`worker_host_model_resolution_postgres_evidence.json`](mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json) (`6cef1b8`, hardened by `9a7db70`, durable adapter completion `8ed53df`) closes the local control-plane model edge: an approved Model Profile/version is resolved deterministically, persisted as a snapshot, request-mismatched snapshots are rejected before claim, and a valid snapshot is carried through the committed worker-host request/run and matched to exact provider/model usage after Postgres reopen. The certificate now registers and dispatches the production `GatewayWorkerAdapter` through one bounded local gateway double, while retaining payload-free evidence and zero-row cleanup. The pre-terminal controller guard in `199eb5b` also rejects missing, incomplete, or mismatched result attribution before usage/terminal evidence persistence, while preserving legacy/native no-snapshot compatibility. The provider and model are local fixture identifiers; external provider execution, independent hosts, hardened sandbox operation, and provider-backed recovery remain separate gates.

The explicit worker-plane provider boundary `f999695` is implemented in
...
| Kata Containers | **Canonical `vm_isolated` provider** | The current WSL2 development host has pinned runtime-rs/QEMU registration and a passing guest smoke; native release activation still requires the separate VM-capable host evidence. The host selects the VMM/profile. |
...
Redis is for streams, bounded cache, leases, and ephemeral coordination—not canonical long-term memory. Letta and Qdrant may enrich worker memory through certified adapters, but cannot bypass project scope, retention, deletion, or evidence rules. The current Qdrant and Temporal contracts (`mas_core.memory.optional_services`) enforce exact adapter versions, opaque project namespaces, bounded request values, checksum-only workflow signals, stable provider-failure mapping, and scalar/payload-free results; external services remain opt-in until operator-owned live value, outage, backup, and removal evidence is available.
...
legacy run-correlated fallback, PM inbound metadata, and the durable
  ...
  The checker can also perform an optional signed, trace-filtered identity
  dashboard read-back (`074ef8a`). The bounded `aiat.trace-incident.v1`
  projection/checker (`c357fdf`) now classifies scalar failure findings while
  keeping partial coverage descriptive. Commit `b4b7cef` adds the bounded
  operator-only incident API and dashboard `/logs?trace_id=…` deep link without
  payloads or new gates. Commit `869202c` renders the bounded finding
  references/timestamps in the same read-only deep link. Selected-worker live
  evidence, configured provider callbacks, production retention enforcement,
  complete
  mail-span coverage, and richer live incident chronology are still P2 work.
  The deterministic `aiat.worker-mail-edge-coverage.v1` join (`1d8aed5`)
  composes the independent worker source and mail-edge evaluators with an
  explicit worker/trace scope, requiring worker usage/artifact/model/worker
  sources (plus integration sources when requested), a verified webhook, and a
  bounce/failure signal. It returns counts and missing-signal names only and
  does not select, activate, dispatch, or certify a live worker; its fixture
  evidence is [`worker_mail_edge_coverage_fixture.json`](mas/docs/provenance/worker_mail_edge_coverage_fixture.json).
  The local gateway-worker composition certificate (`6ebb12c`) runs the real
  `GatewayWorkerAdapter`/`WorkerRunController`, records exact fixture
  provider/model usage, and evaluates the scalar worker sources with verified
  delivered and bounced observations; its evidence is
  [`gateway_worker_mail_edge_fixture.json`](mas/docs/provenance/gateway_worker_mail_edge_fixture.json).
  This remains local composition evidence only and does not claim durable
  provider callback/read-back, external provider execution, live worker
  dispatch, or sandbox operation.
  ...
  fixture model/usage attribution and payload-free scalar worker/model trace
  coverage; its artifact is a synthetic report pointer without generated
  content. Durable host storage, independent hosts, external providers,
  sandbox execution, and live recovery remain open.
  The companion host-failure certificate (`2abc02a`) drives retryable `429`
  and permanent `401` gateway cases through the same real boundary. Both
  settle `FAILED`, release their committed binding/reservation, and retain only
  status/cause metadata; automatic live retry and provider recovery remain
  unproven.
  The local
  ...
  contract and deterministic in-memory preview/apply rehearsal; `57e13cb`
  ...
  read adapter (`67f5eae`). `96f5fc0` adds a local Postgres adapter and
  reserved-fixture certificate with one guarded delete, two held rows, and
  zero rows after cleanup; evidence is
  [`trace_retention_execution_live.json`](mas/docs/provenance/trace_retention_execution_live.json).
  Production hold authority, archive, erasure, project narrowing, durable
  audit, provider recovery, and restore parity remain separate live gates.
...
Kata is the active `vm_isolated` implementation, but it is not a current native
release prerequisite. The current WSL2 development host has pinned runtime-rs/
QEMU inputs, Docker registration, and a distinct guest-kernel smoke. Native
Linux host certification must still prove `/dev/kvm` or another supported
virtualization path, the selected Kata runtime/profile, network and filesystem
restrictions, cleanup, and recovery before native `vm_isolated` release
activation. Use `scripts/check_sandbox_runtime_readiness.py --live
--require-kata` for the explicit host probe.

### 12.3 Network architecture

Team runners communicate only with the router, tool service, orchestrator, and approved gateway endpoints on the worker network. They do not connect directly to Redis, Postgres, PgBouncer, object storage, identity database, or provider APIs. The checked-in `aiat.network-boundary-policy.v1` matrix at `mas/docs/provenance/network_boundary_policy.yaml` is the source for the static Compose and live deny/allow target set; the checker also rejects host-published runner ports and non-internal worker networks. Checkpoints, usage events, documents, and review metadata cross the data-plane boundary through the typed, operation-allowlisted control-plane storage API; a runner fails startup if that durable path is unavailable.
...
- host lease-generation fencing: replacement registration and expired-host
  reconciliation must fence stale heartbeats and expire reservations from the
  lost host incarnation before placement can consider a replacement;
...
The AIAT-owned encrypted backup envelope is implemented at
[`mas/packages/mas-core/mas_core/memory/object_store_encryption.py`](mas/packages/mas-core/mas_core/memory/object_store_encryption.py)
with the deterministic certificate
[`mas/scripts/check_object_store_encryption.py`](mas/scripts/check_object_store_encryption.py)
and retained scalar evidence
[`mas/docs/provenance/object_store_encryption_evidence.json`](mas/docs/provenance/object_store_encryption_evidence.json).
Plaintext is encrypted before a backup adapter receives it; manifests retain
only opaque key IDs, nonces, checksums, sizes, and content type. This is a
provider-neutral local envelope certificate, not provider-managed SSE/KMS,
external-backend, clean-host, or disaster-recovery evidence. The follow-on
[`mas/scripts/check_object_store_clean_environment_restore.py`](mas/scripts/check_object_store_clean_environment_restore.py)
certificate persists only ciphertext and scalar manifest metadata, restores
through a distinct fresh Python process and fresh adapters, and removes its
temporary bundle; scalar evidence is retained at
[`mas/docs/provenance/object_store_clean_environment_restore_evidence.json`](mas/docs/provenance/object_store_clean_environment_restore_evidence.json).
It closes the local clean-process prerequisite only; provider-pair, KMS,
clean-host, outage, and disaster-recovery evidence remain separate.
...
Its guarded `--live` rehearsal requires two explicit S3-compatible endpoints,
a reserved `aiat-migration-live-*` project, fixture seeding, and separate human
confirmations for cutover and rollback. The disposable MinIO/SeaweedFS run
passes inventory, checksum/read-back copy, one dual write, the AIAT-owned
...
The bounded resource profile in `3791b3f` now also passes eight 1 MiB/8 MiB,
concurrency-four checksum cases per endpoint with scalar procfs RSS and
wall/CPU observations and zero cleanup; evidence is retained at
[`mas/docs/provenance/object_store_resource_profile_provider_diverse_evidence.json`](mas/docs/provenance/object_store_resource_profile_provider_diverse_evidence.json).
These observations do not set production budgets or select a provider. The
retained local MinIO report proves the scoped 8/8 contract on the running
...
A deployed MinIO/SeaweedFS adapter still needs broader production resource,
portability, outage,
...
optional dual-write parity, and human-confirmed cutover/rollback decisions. The
guarded live checker deletes only its reserved fixture objects after verifying
cleanup; it does not silently mutate deployment routing or retention authority,
and provider configuration remains an explicit operator action.
...
prove scope, a typed authoritative hold snapshot, typed
checksum/count/clean-target backup/read-back parity, and human confirmation
before one atomic action batch.
The current in-memory adapter is a deterministic rehearsal only. Worker/runtime deletion must
...
- [x] Certify the default Cloudflare Email Routing + Worker/D1-only inbound and
  ...
  `Docs/AIAT_Email_Identity_Live_Certification.md`.
- Rehearse optional mail-profile outage, restore, key rotation, and domain
  migration behavior when that scope is selected.
...
- [x] Add the bounded native transport/model/tool/audit/worker/integration
  ...
  deterministic redaction fixture; identity mail-edge spans and live
  retention/sampling enforcement remain open and non-gating.
- [x] Add the payload-free `aiat.trace-incident.v1` summary and deterministic/
  fail-closed checker (`c357fdf`); scalar failure references and partial/empty
  coverage are descriptive.
- [x] Expose the incident projection through the operator-only
  ...
  population and richer chronology remain separate evidence work.
...
- [x] Certify one real `WorkerRunController`/`NativeWorkerAdapter` execution
  ...
  connection, payload-free trace projection, and reserved-fixture cleanup.
  ...
  callback/bounce, sandbox, canary/rollback, retention, and outage evidence
  remain separate.
...
- [x] Add the provider-neutral `aiat.trace-retention-execution.v1` contract
  ...
  `67f5eae` adds the provider-neutral registry read adapter. `96f5fc0` adds
  `PostgresNativeTraceRetentionStore` and a local `--live` certificate for
  reserved native-span backup/read-back, human-confirmed trace-scoped delete,
  held-row preservation, and cleanup; evidence is
  [`trace_retention_execution_live.json`](mas/docs/provenance/trace_retention_execution_live.json).
  Production registry authority, archive, erasure, durable audit, provider
  recovery, and restore rollback remain separate gates.
...
- Certify optional Letta, Qdrant, and Temporal integrations behind AIAT authority. The static catalogue/checker and bounded Qdrant/Temporal contracts are implemented (`0f1db19`); external-service live value, outage, backup, removal, and production certification remain separate gates.
...
- [x] Extend the object-store benchmark to a bounded 1 MiB/8 MiB wave with
  ...
  The bounded scalar resource profile is checked separately; production
  resource budgets/portability, outage, provider-managed encryption, and
  clean-host/disaster-recovery evidence remain separate gates.
- [x] Add the bounded multipart object-store adapter/checker (`a2f35de`) for
  ...
  The bounded scalar resource profile is checked separately; production
  resource budgets/portability, provider-outage, provider-managed encryption, clean-host,
  disaster-recovery, and production-cutover evidence remain separate gates.
- [x] Add the scalar object-store resource profile (`3791b3f`) over the bounded
  1 MiB/8 MiB, concurrency-four checksum wave, retaining procfs RSS and
  wall/CPU scalars with cleanup verification in
  ...
  Production resource budgets/portability, outage, provider-managed
  encryption, clean-host, disaster-recovery, and production-cutover evidence
  remain separate gates.
...
2. **Implemented:** reconcile the prior coding/tester `approved` status with security evidence: the historical OpenCode source scan is retained only as `FAILED_UNREPRODUCIBLE`, the fresh pinned candidate workflow records immutable provenance and scanner-error separation, both manifests remain pending/non-activatable, and activation fails closed until the fresh scan passes.
...
10. **Implemented preparatory contract export:** commits `66b8690`, `f2b0961`, and `fd61456` plus the bounded artifact/usage evidence-read group `2ca5f3d`, diagnostics group `2860838`, retention-plan group `f8829d6`/`b3fca97`/`9a80c6c`, and protocol-schema reconciliation `8f46ed1` keep `schemas/http/orchestrator.openapi.json`, the checked-in `aiat.v1` protocol schema, deterministic dashboard TypeScript, and internal Python SDK models/operation metadata aligned through `scripts/check_api_contract.py`. The current export contains 238 paths, 137 schemas, and 271 operations, including the typed read-only retention-plan response and legal-hold fields; `8f46ed1` also reconciles the checked-in `WorkerManifest.transport` enum with the runtime `aiat_gateway` transport and records the matching protocol provenance hash. External-language SDKs and broader modular router extraction remain P1 work after the P0 exit.
...
3. Use the certified default identity/mail path; complete optional key/domain
   migration and mail outage/restore rehearsal if that scope is selected.
...
- `mas/scripts/check_worker_run_postgres_evidence.py`,
  `mas/scripts/tests/test_check_worker_run_postgres_evidence.py`, and
  `mas/docs/provenance/worker_run_postgres_evidence.json` — the real local
  Postgres controller/native-adapter durability certificate (`acd3f06`) with
  second-connection read-back, payload-free trace coverage, and scoped
  cleanup; external model/provider execution remains a separate gate.
...
- `mas/scripts/check_worker_multi_host_execution_postgres.py` and
  `mas/docs/provenance/worker_multi_host_execution_postgres_evidence.json` —
  local two-host WorkerHostExecutor certificate extended by `d45e4dd` with a
  raced duplicate-claim rejection, exactly two adapter dispatches, terminal and
  alias replay without redispatch, Postgres reopen, and scoped cleanup;
  independent hosts, providers, and sandbox recovery remain separate gates.
...
- `mas/packages/mas-core/mas_core/observability/native_spans.py`,
  ...
  production retention authority/audit/erasure/restore remain open.
...
- `mas/packages/mas-core/mas_core/observability/worker_trace_coverage.py`,
  `mas/scripts/check_worker_mail_edge_coverage.py`, and
  `mas/docs/provenance/worker_mail_edge_coverage_fixture.json` — deterministic
  `aiat.worker-mail-edge-coverage.v1` composition of worker source counts and
  explicitly scoped, payload-free delivery/webhook/bounce observations
  (`1d8aed5`); live worker/provider evidence remains separate.
- `mas/scripts/check_worker_run_postgres_evidence.py` and
  `mas/docs/provenance/worker_run_postgres_evidence.json` — durable local
  worker-run lifecycle/evidence read-back at migration `0036_native_trace_spans`
  with payload-free model/worker/audit span coverage and reserved-fixture
  cleanup (`acd3f06`); live provider, sandbox, canary, retention, and recovery
...
- `mas/packages/mas-core/mas_core/observability/retention_execution.py`,
  `mas/packages/mas-core/mas_core/observability/postgres_retention.py`,
  ...
  `96f5fc0` adds the local Postgres adapter and retained certificate at
  [`mas/docs/provenance/trace_retention_execution_live.json`](mas/docs/provenance/trace_retention_execution_live.json);
  production registry/storage authority, durable audit, erasure, archive,
  provider recovery, and restore rollback remain separate gates.
...
- Canonical runtime catalogue in `mas/packages/mas-core/mas_core/worker_registry/runtime_catalog.py`; checked-in manifest reconciliation currently reports 39 manifests, with coding/tester security evidence pending the fresh pinned OpenCode candidate scan.
- OpenCode candidate interface version 1.18.21 with recorded exact commit/image
  provenance; security/SBOM certification remains blocked and workers stay
  inactive until it passes.
...
- Current default security choices: Semgrep CLI, SkillSpector, and gVisor for
  `sandboxed` workers; Kata is the `vm_isolated` provider for high-risk or
  gVisor-incompatible workers when the host passes its guest smoke, while
  direct Firecracker remains compatibility-only.
