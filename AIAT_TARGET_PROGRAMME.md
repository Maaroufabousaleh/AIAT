...
The deterministic placement certificate [`worker_placement_contract.json`](mas/docs/provenance/worker_placement_contract.json) adds the AIAT-owned `aiat.worker-placement.v1` predicate over explicit host snapshots: readiness/lease, labels, capabilities, sandbox/isolation, capacity, deterministic ordering, and duplicate-ID rejection are checked without mutation. Lease reservation/commit, live multi-host scheduling, host-loss/split-brain, and Firecracker evidence remain open.
The durable host certificate [`worker_host_registry_postgres_evidence.json`](mas/docs/provenance/worker_host_registry_postgres_evidence.json) now proves AIAT-owned host registration with token-digest authentication, wrong-token rejection, heartbeat lease renewal, redacted host/public placement snapshots, connection-reopen read-back, and expired-lease visibility at migration `0037_worker_host_registry`. It does not claim capacity reservation/commit, durable host recovery, live multi-host scheduling, host-loss/split-brain avoidance, or Firecracker certification.
