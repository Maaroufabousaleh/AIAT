...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("comments", sa.JSONB(), comment="List of {section, comment, suggested_fix}"),
        sa.Column(
            "submitted_at",
            sa.TIMESTAMP(timezone=True),
            server_default=sa.text("now()"),
    ...
    op.create_table(
        ...
        sa.Column("assignee_team", sa.Text(), comment="team_id — which team handles this issue"),
        sa.Column("assignee_agent", sa.Text(), comment="agent_id if directly assigned"),
        sa.Column("estimated_hours", sa.Numeric(10, 2)),
        sa.Column("actual_hours", sa.Numeric(10, 2)),
