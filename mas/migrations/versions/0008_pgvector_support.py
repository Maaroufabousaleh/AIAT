...
down_revision = "0007_worker_integration"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        USING hnsw (content_vector vector_cosine_ops m = 16, ef_construction = 64)
    """)

    op.execute("""
        CREATE INDEX IF NOT EXISTS idx_chunks_project_vector 
        ON project_context_chunks (project_id, content_vector) 
        USING hnsw (content_vector vector_cosine_ops m = 16, ef_construction = 64)
    """)


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS idx_chunks_project_vector")
