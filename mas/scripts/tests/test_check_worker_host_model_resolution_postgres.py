...
SCRIPT = Path(__file__).resolve().parents[1] / "check_worker_host_model_resolution_postgres.py"


def _module():
    spec = importlib.util.spec_from_file_location(
        "check_worker_host_model_resolution_postgres", SCRIPT
    )
...
def test_checker_blocks_without_database_dsn() -> None:
    ...
    assert report["schema_version"] == (
        "aiat.worker-host-model-resolution-postgres-certification.v1"
    )
...
def test_fixture_contract_is_model_resolution_bound() -> None:
    module = _module()
    assert module.PROVIDER_ID == "fixture-provider-gateway"
