...
SCRIPT = Path(__file__).resolve().parents[1] / "check_worker_multi_host_execution_postgres.py"


def _module():
    spec = importlib.util.spec_from_file_location(
        "check_worker_multi_host_execution_postgres", SCRIPT
    )
...
def test_checker_blocks_without_database_dsn() -> None:
    ...
    assert report["schema_version"] == (
        "aiat.worker-multi-host-execution-postgres-certification.v1"
    )
