...
| PR | Exact paths and purpose | Schema change | Tests and acceptance criteria | Dependencies and rollback | Code that becomes obsolete | Codex versus operator |
| --- | --- | --- | --- | --- | --- | --- |
| **PR H — OpenCode/OpenHands benchmark (corpus/run-plan complete; execution pending)** | Existing [`runtime_adapters.py`](../packages/mas-core/mas_core/worker_registry/runtime_adapters.py), [`openhands_agent_server_adapter.py`](../packages/mas-core/mas_core/worker_registry/openhands_agent_server_adapter.py), OpenCode evidence under [`mas/docs/opencode/`](opencode/), OpenHands evidence under [`provenance/openhands-candidate/`](provenance/openhands-candidate/), and the dependency-free fixed corpus [`corpus.json`](../scripts/fixtures/coding-runtime-benchmark/corpus.json) validated by [`check_coding_runtime_benchmark.py`](../scripts/check_coding_runtime_benchmark.py). | No canonical schema change; benchmark result artifacts remain future evidence under `mas/docs/provenance/`. | The 40-task/160-run plan, category counts, required observations, no-network/no-credential/no-external-effect policy, and no-winner invariant pass locally. Live execution still requires 80 runs per candidate with the same profile/workspace/grants; test/diff/artifact/evidence correctness, security, cost, latency, restart/cancel, and OpenHands certification remain open. | Depends on operator runtime/provider availability and the OpenHands live-certification gate. Rollback is a worker-manifest/default flip to OpenCode; no project-state migration. | Losing runtime's adapter may become removable only after a sustained rollback window and proof of no active workload. | **Codex implemented and tested the corpus/run-plan contract.** Operator supplies live runtime/provider, sandbox hosts, and final default decision. |
| **PR I — Pydantic AI benchmark (corpus/run-plan complete; execution pending)** | [`agent_runtime/base.py`](../packages/mas-core/mas_core/agent_runtime/base.py) remains the baseline; the dependency-free fixed corpus [`corpus.json`](../scripts/fixtures/pydantic-agentbase-benchmark/corpus.json) and validator [`check_pydantic_agentbase_benchmark.py`](../scripts/check_pydantic_agentbase_benchmark.py) are now present. No Pydantic AI dependency was added and no AgentBase default changed. | None for the benchmark. | The 48 cases × 3 runs per runtime / 288 total-run plan, exact category counts, same-profile/tool/context policy, no-activation invariant, and required quality/schema/cost/latency/restart/code-deletion metrics validate locally. Provider execution, qualitative review, and the deletion gate remain open. | Depends on E and G plus provider/operator availability; install/use only in an explicit experimental profile. Remove the experiment and restore the current AgentBase binding if it fails. | `AgentBase` generic loop/tool/checkpoint glue becomes a deletion candidate only if the gate passes. | **Codex implemented and tested the corpus/run-plan contract.** Operator supplies provider credentials and judges quality/acceptance where tests are insufficient. |
| **PR J — Browser benchmark (corpus/run-plan complete; execution pending)** | Existing tool service under [`mas/apps/tool-service/tool_service/`](../apps/tool-service/tool_service/) and its browser/MCP policy boundary; the dependency-free fixed corpus [`corpus.json`](../scripts/fixtures/browser-runtime-benchmark/corpus.json) and validator [`check_browser_runtime_benchmark.py`](../scripts/check_browser_runtime_benchmark.py) are now present. No production Stagehand module was added and Playwright remains the default. | None. | The 30-workflow/180-run plan, exact category counts, deterministic-first fallback policy, disposable/no-credential/no-external-effect rules, duplicate-effect/cross-origin gates, and required reliability/cost/latency/evidence metrics validate locally. Browser execution, dynamic-site evidence, and any agentic fallback decision remain open. | Depends on tool policy and identity fixtures plus operator-selected browser environments. Disable the experimental fallback to return to Playwright-only. | Stagehand/browser-agent integration is deleted if Playwright meets the target or the fallback fails the gate. | **Codex implemented and tested the corpus/run-plan contract.** Operator supplies real authenticated sites/accounts and approves consequential-action tests. |
