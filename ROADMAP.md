...
A fresh unconfigured `--live --json` aggregate at `2026-08-19T04:27:30Z`
against revision `627f7bd` records 69 passes, zero failures, 14 externally
blocked checks, and four pending evidence items out of 83. The two newly
registered lifecycle/retention-execution rows pass statically; live blockers
remain native host/runtime/image prerequisites, missing orchestrator
configuration, relay/source selection, and the open security review. The
aggregate is scalar-only and retains no payloads, credentials, logs, or
disposable state; it remains descriptive with `NO-RELEASE`.
...
The release-inventory follow-up (`aa7d2b2`) now executes both the object-store
lifecycle fixture and the trace-retention execution/project-scope fixture as
machine-readable recovery checks. The static aggregation is 59/59 pass, while
live hold authority, provider retention parity, erasure, and restore gates
remain separate.
