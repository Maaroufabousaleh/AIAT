...
| --- | --- | --- |
| R0 — documentation authority | complete | Canonical target, eleven feature documents, three plans, root navigation, and the personal/internal metadata-only policy are present. Both the current workspace and a clean Git archive pass `check_docs_index.py`; the generated/ignored `mas/uv.lock` remains a local runtime input rather than a documentation link. |
