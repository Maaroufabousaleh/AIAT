...
The latest grouped verification (`84ec4ff`, based on the workflow-control
...
maintained feature/plan/status docs, and the existing network/sandbox
evidence; current ledger snapshot `906c0f9`, 2026-08-18) keeps the static
release ledger at 51/51 pass and confirms the protocol, documentation-index,
provenance, image-inventory, and host-restart-boundary checks. The native
release-host, durable worker, sandbox, pending security review, deployment
image artifacts, and clean-worktree gates remain open. The workflow-control
fixture extension (`9972b3b`) now also covers deterministic cancellation,
review-circuit escalation, node timeout, and invalid-transition rejection;
native/live transition history and cold recovery remain open. The local
durable recovery certificate also verifies superseded retry history,
flow-switch context preservation, cancelled-instance retry, connection-
reopen read-back, and scoped cleanup. The multi-host extension (`d45e4dd`)
adds a local race certificate for one rejected duplicate claim, exactly two
adapter dispatches, and terminal/alternate-ID replay without redispatch;
independent deployed-host/process, provider, sandbox, and complete-version-pin
