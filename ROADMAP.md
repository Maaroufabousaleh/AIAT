...
The canonical OSS architecture, simplification, benchmark, and implementation
plan is [`mas/docs/AIAT_OSS_ARCHITECTURE_AND_IMPLEMENTATION_PLAN.md`](mas/docs/AIAT_OSS_ARCHITECTURE_AND_IMPLEMENTATION_PLAN.md).
It is authoritative for OSS decisions and migration sequencing; the target
...
it does not replace the detailed feature specifications or release ledger.

The current core migration head is `0045_worker_tool_effects`. The
project-transition notification intent is persisted with project state and
history and drained by a bounded stable-ID dispatcher; specialist worker
runtime references are now persisted per dispatch attempt for restart lookup;
OpenCode and the inactive OpenHands candidate consume those references for
bounded subordinate status observations, while the controller records the
observation without changing canonical run state. Compatible runtime-
reconciliation and cancellation-receipt hooks remain while adapter-specific
termination/result recovery and live evidence are open; live outage/restart
evidence remains an R1 release gate. See the [implementation index](Docs/current/AIAT_IMPLEMENTATION_INDEX.md#current-capability-index)
for the maintained feature/status/evidence map.

The governance-agent plane now has an additive, project-scoped model snapshot
carrier: snapshot-bearing messages are resolved through the TeamRunner
control-plane storage boundary and bound to `AgentBase` for that dispatch.
When TeamRunner has its control-plane storage boundary, model-bearing messages
without a snapshot are now rejected rather than falling back to a configured or
...
| Area | Current state |
| --- | --- |
| Integration baseline | Kata canonical `vm_isolated` architecture, host bootstrap, runtime registration, and live gVisor/Kata evidence are committed at `570083dc`. The canonical OSS audit remains frozen to its separate audited SHA, and the reviewed OpenHands tip is `15d2c1a874a2748548338066782b89561353de2e`. |
...
| Kata | **`AVAILABLE` for local development** — pinned Kata runtime-rs 4.2.0 with QEMU is registered in the dedicated WSL Docker daemon and the bounded guest smoke proves a distinct guest kernel. Native-Linux release certification remains pending. |
...
`2026-09-19T22:39:32Z` reports 74 pass, 0 fail, 10 blocked, and 5 not-in-scope
...
The read-only `check_clean_candidate_release.py --candidate-sha <sha> --json`
validator confirms an explicitly pinned candidate SHA and retention boundary;
...
digest-pinned smoke, sandbox, cleanup, and zero-residue checks. This closes the
...
The current WSL2 development environment has a Kata runtime-rs/QEMU path
certified for local development, so `vm_isolated` is available locally and
remains fail-closed on hosts that do not pass the guest smoke. Native-Linux
release certification remains separate. Provider-managed KMS/SSE remains required; the default
...
incomplete GitHub result cannot be attached to this candidate. `7b2e60d` and
...
laptop/host-bound gateway targets; and `c7b8460` records run-scoped profile
...
database harness selection for the local worker execution/recovery wave:
fixture labels and deterministic priorities make host-execution, multi-host,
host-fencing, lease, and queued-run loss certificates reproducible with
...
`2026-08-19T00:36:44Z` is 59/59 pass with `NO-RELEASE`. Follow-up
`aa7d2b2` registers the bounded object-store lifecycle and trace-retention
execution checks; the scalar summary is retained at
[`mas/docs/provenance/release_ledger_static_current.json`](mas/docs/provenance/release_ledger_static_current.json).
This changes verifier coverage only and does not waive live release gates.
...
recovery, and provider evidence still require an operator-owned release host.
...
The retained pre-certification live-gate probes failed closed at the correct
operator boundary: outbound mail required operator-owned relay credentials plus
a safe recipient, and self-improvement candidate detection required an
explicitly selected signal source/project scope. The default Cloudflare/Resend
email identity path is now live-certified; self-improvement still requires its
operator-selected signal source. Their fixtures and corrected 56-test
...
A fresh unconfigured `--live --json` aggregate at `2026-08-19T04:47:35Z`
against revision `57504a7` records 71 passes, zero failures, 14 externally
blocked checks, and four pending evidence items out of 85. The lifecycle,
retention-execution, encrypted-backup, and clean-process-restore rows pass
statically; live blockers remain native host/runtime/image prerequisites,
missing orchestrator configuration, relay/source selection, and the open
security review. The aggregate is scalar-only and retains no payloads,
credentials, logs, or disposable state; it remains descriptive with
`NO-RELEASE`.
...
The mail-boundary regression slice is also reconciled: `3edff39` fixes the
JMAP relay-verifier fixture contract, case-safe SMTP traceback provenance, and
unprivileged security-artifact test modelling; the mail-edge suite passes
11/11 and the SMTP-gateway suite passes. `35e52e1` records that verification;
the bounded default Cloudflare/Resend email identity certification is recorded
in `Docs/AIAT_Email_Identity_Live_Certification.md`, while optional-profile and
broader provider outage/restore evidence remain separate operator gates.
| R6–R7 — P2 scale and guarded autonomy | partial | Local MinIO conformance and same-provider backup/restore pass; restore copies now fail closed on non-empty target prefixes (`93bf755`); a bounded local `time_now` run now proves project-usage plus `tool_service` native trace read-back; the payload-free `aiat.mail-edge-observation.v1` normalizer, coverage evaluator, and fail-closed checker are implemented in `85369fe`; identity-service migration `0003_mail_edge_observations`, signed delegated webhook persistence, event-ID conflict handling, and scalar dashboard/trace/SLO projection are implemented in `cfafe38`; the Resend/Svix raw-body verifier and provider-facing ingress route are implemented in `2d21a2f`, with projected provider-span classification in `29d4da5`, optional signed identity dashboard read-back in `074ef8a`, and local ASGI ingress certificates for in-memory and rebuilt Compose Postgres profiles in `aab6285` and `2d04b30`; the deterministic worker↔mail-edge evidence join and certificate are implemented in `1d8aed5`; the real worker controller/native adapter now has a local Postgres durability, connection-reopen, payload-free trace, and scoped-cleanup certificate in `acd3f06`; bounded trace incident summary/checker in `c357fdf`, and the operator API/generated-contract/dashboard `/logs?trace_id=…` deep link in `b4b7cef`; authenticated self-improvement lifecycle persistence, revision/CAS actions, typed references, and bounded outcomes are committed in `64218ab`, and the complete bounded lifecycle now has a local Compose Postgres certificate for six technical gates, stale-write rejection, human approval, five artifact read-backs, exact rollback, durable outcome/history, and scoped cleanup in `10983c8`; the durable host registry/reservation/scheduler chain now has local Compose certificates through `d9917f8`, durable Worker Run host assignment is certified by `08f1610e`, the committed-binding local native host executor is certified by `73c0bda`, selected Model Profile resolution/snapshot propagation plus pre-claim snapshot consistency are certified by `6cef1b` and `9a7db70`, and the bounded AIAT gateway worker adapter fixture plus manifest/catalogue/reconciliation registration, lifecycle/input hardening, loopback HTTP client-boundary/retry certificate, local worker/mail-edge composition, gateway failure classification, bounded host-executor composition, and host-boundary failure settlement are certified by `080ee18`, `f6baebc`, `cec1e4c`, `cbbfe56`, `6ebb12c`, `b2ae516`, `38c99f4`, and `2abc02a`; `0e0a76f` now certifies the raw-provider Resend/Svix composition and durable provider-message worker/trace correlation through both Postgres stores; `17f6547` plus retained live evidence [`gateway_worker_provider_mail_edge_live.json`](mas/docs/provenance/gateway_worker_provider_mail_edge_live.json) now certify one selected live worker/provider call, durable dual-Postgres reopen, raw-ingress delivered/bounced read-back, payload-free coverage, and cleanup; `1679341`/`9c7e76d` now certify local durable provider-shaped retry with two adapter attempts, raw-ingress replay/conflict/tamper checks, independent reopen, payload-free projection, and zero-row cleanup in [`gateway_worker_mail_edge_provider_recovery_postgres_evidence.json`](mas/docs/provenance/gateway_worker_mail_edge_provider_recovery_postgres_evidence.json); `cec6558`/`520c6bf` now certify local process-isolated worker execution with two distinct child processes, durable reopen, payload-free trace coverage, and zero-row cleanup in [`worker_independent_process_execution_postgres_evidence.json`](mas/docs/provenance/worker_independent_process_execution_postgres_evidence.json); `6794b9f` now certifies the bounded 1 MiB/8 MiB concurrent object-store benchmark wave across local MinIO/SeaweedFS; `a2f35de` now certifies bounded 5 MiB-part multipart upload/read-back/abort/cleanup across the same endpoints; provider-diverse migration, provider-backed encrypted/clean-host recovery, external provider callback/delivery/outage recovery, independent deployed-host loss/split-brain, resource profiling, gVisor, Firecracker, optional memory services, and live self-improvement worker/provider/control-plane evidence remain later work. |

The current bounded resource-profile wave (`3791b3f`/`c90fc13`) supersedes the
earlier R6 snapshot's “resource profiling” wording: both MinIO and SeaweedFS
pass eight 1 MiB/8 MiB checksum cases at concurrency four with identical
procfs RSS/wall/CPU sampling and zero cleanup, and the same reserved namespace
passes multipart upload/read-back/abort. The corrected provider-process
outage/recovery wave (`d92b3dc`/`407a4f9`) then passes the identical 64 KiB/1 MiB
checksum workload for both providers across a real stop/restart boundary with
zero object, container, helper, and disposable-volume residue. Production
resource budgets, portability, provider-managed KMS, clean-host, and
disaster-recovery evidence remain open. The release ledger now validates the
...
certificate (`12ba7c7`/`1808a15`, scalar refresh `5da200f`) separately passes
ciphertext-at-rest,
...
Postgres (refreshed 2026-08-21T17:49:10Z, evidence `4c6f400`). It advances the
...
The release-inventory follow-up (`aa7d2b2`) executes both the object-store
lifecycle fixture and the trace-retention execution/project-scope fixture as
machine-readable recovery checks; `57504a7` adds the encrypted-envelope and
fresh-process restore prerequisites with scalar-only summaries. The current
static aggregation is 63/63 pass, while live hold authority, provider
retention parity, erasure, provider-managed KMS, clean-host, and disaster-
...
zero cleanup. Provider-diverse migration, actual provider outage, KMS,
...
candidate contract (`2760c7a`/`5d62ca6`) for Letta, Qdrant, and Temporal, plus
bounded injected-backend Qdrant/Temporal adapters (`0f1db19`). The adapters
require exact versions, opaque project namespaces, bounded inputs, and
scalar-only results; they do not import, enable, or contact optional services.
The Letta shell's unavailable/error output is now payload-free and scalar-only
...
Live value and recovery evidence remains open.

The provider-diverse follow-up (`f52fde6`) exercises the same boundary between
Compose MinIO and disposable SeaweedFS 4.42, and runs the bounded benchmark at
1/32/4096-byte payload sizes on both endpoints. The pair and all six benchmark
cases pass checksum read-back and scoped cleanup. This is operator-observed
endpoint/comparison evidence only; provider durability/custody, KMS, actual
outage, clean-host/disaster recovery, and migration
cutover remain later gates. Commit `6794b9f` adds a separate bounded advanced
wave at 1 MiB/8 MiB with four concurrent cases per size on both endpoints;
sixteen cases pass checksum read-back and cleanup, retained at
...
This closes only the bounded large-object/concurrency comparison; the scalar
resource profile is checked separately, while production resource budgets,
portability, outage, provider-managed encryption/KMS, clean-host, and
disaster-recovery gates remain open.

Commit `a2f35de` adds the bounded multipart adapter boundary and checker. Both
local MinIO and disposable SeaweedFS pass 8 MiB and 16 MiB payloads with 5 MiB
parts, checksum read-back, explicit abort-without-object, and zero cleanup;
scalar evidence is retained at
...
The scalar resource profile is checked separately; production resource
budgets/portability, outage, provider-managed encryption/KMS, clean-host,
disaster-recovery, and migration-cutover gates remain open.
...
The guarded migration rehearsal (`ecbef00`) now runs the AIAT-owned migration
workflow across the same disposable MinIO/SeaweedFS topology. It requires a
reserved `aiat-migration-live-*` project, explicit fixture seeding, and human
confirmation for both cutover and rollback; three-object inventory,
checksum/read-back copy, one dual write, `CUTOVER` → `ROLLED_BACK` history, and
scoped cleanup pass with zero remaining objects. Evidence is retained at
...
This is a reversible workflow rehearsal only: deployment routing, retention
authority, provider outage, KMS, clean-host, and disaster-recovery evidence
remain later gates.

The current storage benchmark implementation is therefore explicit about its
scope: serial small-object baselines, one bounded larger/concurrent wave, and
one scalar RSS/wall/CPU resource profile are retained as local evidence, while
no provider-selection or production routing decision is implied. The earlier
nonexistent-network-alias attempt is harness/configuration-invalid and is not
part of the provider verdict.

The latest grouped verification through `f3d4e7b` (including the bounded
...
`6794b9f`, advanced evidence/docs group `d0e4d19`, multipart implementation
`a2f35de`, multipart evidence/docs group `d39adce`/`e804424`, the status-doc
refresh `a19aff1`, and the refreshed release ledger `f3d4e7b`; based
...
current ledger snapshot [`f3d4e7b`](mas/docs/AIAT_CURRENT_RELEASE_LEDGER.md),
2026-08-18T14:08:29Z) recorded
the historical static release ledger at 52/52 pass and the
historical unconfigured local aggregate at 55 pass, 0 fail, 16 blocked, and 4 pending with
`NO-RELEASE`. It confirms the protocol, documentation-index, provenance,
image-inventory, and host-restart-boundary checks. The native release-host,
durable worker, sandbox, pending security review, deployment image artifacts,
and clean-worktree gates remain open. The workflow-control fixture extension
(`9972b3b`) now also covers deterministic cancellation, review-circuit
escalation, node timeout, and invalid-transition rejection; native/live
transition history and cold recovery remain open. The local durable recovery
certificate also verifies superseded retry history, flow-switch context
preservation, cancelled-instance retry, connection-reopen read-back, and
scoped cleanup. The multi-host extension (`d45e4dd`) adds a local race
certificate for one rejected duplicate claim, exactly two adapter dispatches,
and terminal/alternate-ID replay without redispatch. The version extension
(`7c1ef74`) adds model-profile/snapshot pins and complete local governed
version read-back. The gateway recovery extension (`1679341`, `9c7e76d`)
adds two-attempt local provider-shaped retry with dual-Postgres reopen,
raw-ingress replay/conflict/tamper checks, payload-free projection, and
cleanup. The process-boundary extension (`cec6558`, `520c6bf`) launches two
separate Python child processes that settle separate durable bindings, then
verifies distinct process IDs, reopen, payload-free traces, and cleanup;
...
The same-host recovery soak is deliberately additive evidence: three separate
child processes repeat the production host-loss/requeue/reassignment checker
against local Compose Postgres, with durable reopen, scalar-only projection,
and zero fixture rows per iteration. It does not close independent deployed
host loss/split-brain, provider outage, sandbox, deployment load/chaos, or
...
preflight, and zero-row cleanup. The follow-on groups `b0f27f6` and `59294c0`
add a fresh-process certificate: only ciphertext and scalar manifest metadata
are persisted, a distinct Python process restores through fresh adapters and
the production encrypted-restore helper, and all temporary fixtures are
removed. Provider-managed SSE/KMS, external backend, key custody,
provider-pair, clean-host/disaster-recovery, and outage evidence remain open.
...
documentation and live snapshot groups, 2026-08-18) keeps the historical static
release ledger at 52/52 pass and confirms the protocol, documentation-index,
provenance, image-inventory, and host-restart-boundary checks. The selected
live provider certificate is retained, but this does not advance R1 to
release-ready: the native host, durable worker, sandbox, pending security
review, deployment image artifacts, and clean-worktree gates remain open. The
durable selected-model host certificate remains bounded local-gateway evidence
and does not close the live provider or sandbox gates.

The opt-in worker-plane provider boundary `f999695` is now implemented in
[`check_gateway_worker_provider_live.py`](mas/scripts/check_gateway_worker_provider_live.py).
It fail-closes by default, requires an exact model observed in the gateway's
read-only `/v1/models` listing plus explicit operator opt-in, and can make one
bounded completion through the real host executor/controller/adapter path while
retaining only scalar status, usage, and error metadata. The retained live
...
`llama-3.3-70b-versatile` completion with `SUCCEEDED` controller settlement and
binding/reservation release. Durable Postgres, independent-host, sandbox,
recovery, and provider mail-edge evidence remain open.

Commit `17f6547` extends the boundary into the durable worker/mail-edge
checker. Its `--live-provider` mode is fail-closed by default, validates the
same exact model listing and operator opt-in, runs through the real gateway
client/controller/Postgres path, redacts generated content before durable
`result_json` persistence, and can combine with `--provider-ingress` for local
durable webhook/bounce read-back. Retained evidence
[`gateway_worker_provider_mail_edge_live.json`](mas/docs/provenance/gateway_worker_provider_mail_edge_live.json)
records one successful configured run with dual-Postgres reopen,
delivered/bounced raw-ingress status, payload-free projection, generated-text
...
`00a468d` hardens automatic fallback routing to catch every
`httpx.TransportError`, not only timeout subclasses, and its focused provider
suite proves a connection outage falls through to a sibling model while
arming cooldown state. The `aiat.gateway-provider-recovery.v1` fixture/checker
added by `48b32ef` then proves primary outage → secondary fallback → primary
recovery with cooldown clearance, without external network, worker dispatch,
or durable state. This closes local routing/cooldown evidence only; durable
external-provider outage recovery remains open.
...
drives the signed `/v1/mail-edge/provider-webhook` route, reopens both stores
independently, verifies exact fixture attribution and payload-free
worker/mail-edge correlation, and leaves zero reserved rows after cleanup. It
does not claim raw external-provider callback, external provider delivery,
selected live worker execution, recovery, or sandbox evidence.

The raw-provider follow-up `0e0a76f` adds a second mode to that checker. It
creates a scoped outbound attempt, drives the real Resend/Svix raw-body route,
derives worker/trace scope from the durable provider-message match, and passes
replay/conflict/tamper, reopen, payload-free join, and cleanup checks. This
closes the local provider-facing application boundary, not an external
provider callback, delivery, recovery, selected worker, or sandbox gate.

The R6 host-loss status is now extended by `72e59ec`: the durable host
registry/reservation/scheduler chain has generation fencing and expired-host
recovery evidence through migration `0039_worker_host_fencing`. Commit `73c0bda`
...
one queued run concurrently through `WorkerHostExecutor`; both runs retain
payload-free usage/artifact/trace evidence through a Postgres reopen and release
...
Commit `424805c` adds a bounded same-host recovery soak. Three separate child
processes invoke the production host-loss checker against local Compose
Postgres; each iteration proves stale-executor rejection, attempt-two retry,
durable reopen, payload-free scalar projection, and zero fixture rows. This is
repeatability evidence only. Independent deployed hosts, provider outage,
sandbox, deployment-wide load/chaos, clean-host, and disaster-recovery gates
remain open.

Commits `6cef1b8` and `9a7db70` add the selected model-resolution host edge: an approved
AIAT Model Profile/version is resolved deterministically, persisted as a
snapshot, and propagated through a committed worker-host run with exact
provider/model usage attribution, Postgres reopen, payload-free evidence, and
scoped cleanup. The provider and model identifiers are local fixtures, so
external provider execution, independent hosts, hardened sandbox operation,
and provider-backed recovery remain open.

Commit `8ed53df` closes the durable adapter gap in that edge: the Postgres
certificate now registers and executes the production `GatewayWorkerAdapter`
through a bounded local gateway double, records one exact gateway call, and
retains the same durable attribution/reopen/cleanup guarantees. This is still
local fixture evidence; external provider execution, independent hosts,
hardened sandbox operation, and provider-backed recovery remain open.

Commits `080ee18`, `f6baebc`, `cec1e4c`, and `cbbfe56` add the bounded `aiat_gateway` model-worker edge:
`GatewayWorkerAdapter` requires a resolved exact model, normalizes bounded
prompt/generation input, emits exact provider/model usage, and completes one
real controller fixture through an in-process gateway double. The transport is
also declared in `WorkerRuntime`, the builtin runtime catalogue, and the
read-only reconciliation checker. The loopback HTTP certificate drives the
real `LLMGatewayClient` through `/v1/chat/completions`, checks bearer-secret
propagation and bounded payloads, retries one fixture `429`, and reads back
terminal usage attribution. The retained fixture evidence records no external
provider call, network mutation, or sandbox execution; live provider-backed
dispatch and hardened sandbox evidence remain open. The owned gateway client
is started/stopped by the adapter, and prompt/message batches and generation
temperature are bounded before dispatch.

`6ebb12c` adds the local gateway-worker/mail-edge composition follow-up. The
real gateway adapter and controller produce exact fixture provider/model usage;
scalar worker/integration sources are then evaluated with verified delivered
and bounced observations. The payload-free, non-mutating certificate is
[`gateway_worker_mail_edge_fixture.json`](mas/docs/provenance/gateway_worker_mail_edge_fixture.json).
It does not claim durable provider callback/read-back, external provider
execution, live worker dispatch, sandbox, or host-runtime evidence.

`fa42284` extends that composition to a durable dual-Postgres certificate, and
`67f1599` adds the signed delegated identity-ingress mode. The
production gateway adapter/controller persists a payload-free worker report,
usage row, and native spans; normalized delivery, verified-webhook, and bounce
observations are persisted by the identity store; both stores are reopened
independently; the signed route checks replay/conflict/tamper behavior; and the
...
`1679341` and `9c7e76d` extend the durable checker’s default fixture profile
with local provider-shaped recovery. One injected transient `429` is retried
once through the production adapter/controller; both Postgres stores reopen
independently, raw-provider ingress replay/conflict/tamper checks pass, the
projection remains payload-free, and cleanup returns zero rows. Evidence is
...
This is not external provider outage/restore, callback/delivery confirmation,
independent host/process, or sandbox evidence.
...
retryable provider failures, and permanent statuses are terminal provider
rejections. Only bounded status/cause-type metadata is retained. This improves
the recovery contract without claiming live provider recovery.

`38c99f4` adds the bounded host-executor composition follow-up. The real
`WorkerHostExecutor`, `WorkerRunController`, and `GatewayWorkerAdapter` admit
a committed in-memory worker-plane binding, claim one queued run, record exact
fixture model/usage attribution, settle and release the binding, and pass
payload-free scalar worker/model trace coverage. The retained artifact is a
synthetic report pointer without generated content; durable host storage,
independent hosts, external providers, sandbox execution, and live recovery
remain open.

`2abc02a` adds the bounded host failure follow-up. The real host/controller/
gateway chain settles both retryable `429` and permanent `401` cases as
`FAILED`, releases their committed bindings/reservations, and retains only
status/cause metadata while excluding provider detail text. Automatic live
retry, external provider recovery, durable host storage, and sandbox evidence
remain open.
...
The latest bounded dashboard increment is dead-letter queue denial-state
recovery: 401/403 read or replay responses now expose a named access-denied
region, retain only previously loaded messages, clear selection/replay state,
and hide refresh/retry, filters, selection, and replay controls while retaining
read-only envelope inspection. Source-built stale, first-load-denial, and
post-read-denial fixture coverage passes 3/3 (`e6ab3a1`).

The latest bounded dashboard increment is Tools catalogue denial-state
recovery: 401/403 responses now expose a named access-denied region, retain
only previously loaded tool metadata, and hide refresh/retry, search, grouping,
expansion, and copy controls while leaving retained tables/details read-only.
Source-built stale, first-load-denial, and post-read-denial fixture coverage
passes 3/3 (`b418f8a`).

The latest bounded dashboard increment is Flows list denial-state recovery:
401/403 reads and deletes now expose a named access-denied region, retain only
previously loaded definitions as read-only text, and hide refresh/retry, New
Flow, search/status filters, selection, editing, and deletion controls.
...
The latest bounded dashboard increment is new-flow builder denial-state
recovery: 401/403 governed worker/profile/template reads and create/readiness
mutations now expose a named access-status region, retain the current draft
canvas as read-only context, and hide templates, palette, node configuration,
...
identity-service migration `0003_mail_edge_observations` now persists one
normalized row per provider event, rejects conflicting replays, correlates
outbound references when possible, and projects provider rows beside delivery
...
checker to classify projected provider webhook spans. `074ef8a` adds optional
signed, trace-filtered identity dashboard read-back without payload retention.
`aab6285` adds the local real-ASGI ingress certificate: the actual identity
service verifies signed delivered/bounced bodies, normalizes and persists only
bounded metadata in a disposable store, rejects duplicate conflicts and body
tampering, and exposes the normalized rows through the dashboard read model.
The retained evidence is
[`mas/docs/provenance/mail_edge_ingress_certification.json`](mas/docs/provenance/mail_edge_ingress_certification.json);
it is local application evidence, not external provider, Postgres, worker, or
live bounce evidence.
`2d04b30` adds the durable local Compose certificate: the rebuilt identity
image reaches migration `0003_mail_edge_observations`, signs the same events
through the real route, reopens `PostgresIdentityStore`, reads two normalized
...
observations, including a verified delivery and bounce signal. The retained
fixture certificate is
[`worker_mail_edge_coverage_fixture.json`](mas/docs/provenance/worker_mail_edge_coverage_fixture.json).
This is an evidence-join contract only; it does not claim live worker,
external-provider, durable-worker, or live-bounce evidence.
...
finding-count, and payload-free finding-reference chronology. `f8829d6` also
...
The Projects list now has an explicit access boundary for its paired project and
active-flow reads plus archive/delete/create mutations: 401/403 responses retain
only previously loaded definitions as read-only text and hide refresh/retry, New
...
The Project Evidence package now has the same canonical-read boundary: a
401/403 package response renders a named denial region, retains any previously
loaded package as read-only context, hides Refresh/Retry and package controls,
and preserves safe back navigation. Its source-built stale, initial-denial,
and retained-read denial matrix passes 3/3 (`00f81b5`).

Evidence Detail now applies the same boundary to bounded scalar reads: a
401/403 response renders a named denial region, retains any last-known scalar
projection as read-only, hides Refresh, and preserves citation identity plus
safe CEO/owning-section navigation. Its source-built scalar, stale,
initial-denial, and retained-read matrix passes 11/11 (`23e2db9`).

The System Overview now applies the same boundary across its seven independent
control-plane/metrics reads: 401/403 responses are classified as access denied,
the named status region identifies denied sources, available values remain
read-only, and retry plus first-run seed actions are hidden until authorization
returns. Its local 403 fixture passes 1/1 (`b0ab779`).
...
| Identity/provider mail-edge observations, local ingress certification, webhook normalization, bounce coverage, and live evidence boundary | [Mail-Edge Observability](Docs/current/FEATURE_MAIL_EDGE_OBSERVABILITY.md) |
...
| Bounded object-store provider-pair recovery and advanced benchmark | [Data, Storage, Memory, and Retention](Docs/current/FEATURE_DATA_STORAGE_AND_MEMORY.md), [Object-Store Migration Review Status](Docs/current/FEATURE_OBJECT_STORE_MIGRATION_STATUS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_object_store_provider_pair.py`](mas/scripts/check_object_store_provider_pair.py), [`check_object_store_benchmarks.py`](mas/scripts/check_object_store_benchmarks.py), [`test_check_object_store_provider_pair.py`](mas/scripts/tests/test_check_object_store_provider_pair.py), [same-provider evidence](mas/docs/provenance/object_store_provider_pair_evidence.json), [provider-diverse evidence](mas/docs/provenance/object_store_provider_pair_provider_diverse_evidence.json), [serial benchmark evidence](mas/docs/provenance/object_store_provider_benchmark_evidence.json), [advanced benchmark evidence](mas/docs/provenance/object_store_benchmark_advanced_provider_diverse_evidence.json); commits `351444a`/`f385bd7`/`3833631` plus `f52fde6` and `6794b9f` certify three-object checksum dual-write/recovery, sixteen bounded 1 MiB/8 MiB concurrency cases, payload-free output, and zero cleanup across local MinIO/SeaweedFS endpoints. Provider durability/custody, resource profiling, actual provider outage, KMS, clean-host, disaster-recovery, and migration cutover evidence remain open |
| Bounded live object-store migration rehearsal | [`check_object_store_migration.py`](mas/scripts/check_object_store_migration.py), [`test_check_object_store_migration.py`](mas/scripts/tests/test_check_object_store_migration.py), [migration review status](Docs/current/FEATURE_OBJECT_STORE_MIGRATION_STATUS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [scalar evidence](mas/docs/provenance/object_store_migration_provider_diverse_evidence.json); commit `ecbef00` | Reserved-prefix MinIO/SeaweedFS rehearsal passes inventory, checksum/read-back copy, one dual write, explicit human-confirmed `CUTOVER` → `ROLLED_BACK` workflow history, payload-free reporting, and four-object-per-side cleanup to zero. Deployment routing and retention authority are unchanged; production cutover, provider outage, KMS, clean-host, and disaster-recovery evidence remain open |
| Encrypted object-store backup and fresh-process restore | [`object_store_encryption.py`](mas/packages/mas-core/mas_core/memory/object_store_encryption.py), [`check_object_store_encryption.py`](mas/scripts/check_object_store_encryption.py), [`check_object_store_clean_environment_restore.py`](mas/scripts/check_object_store_clean_environment_restore.py), [`object_store_encryption_evidence.json`](mas/docs/provenance/object_store_encryption_evidence.json), [`object_store_clean_environment_restore_evidence.json`](mas/docs/provenance/object_store_clean_environment_restore_evidence.json), and [Data/Storage/Memory feature](Docs/current/FEATURE_DATA_STORAGE_AND_MEMORY.md); commits `91504dd`, `b0f27f6`, refreshed scalar evidence `f567979`/`554901c`, inventory registration `57504a7` | AIAT-owned AES-256-GCM encryption before replication, opaque key-ID-only manifests, ciphertext checksum/read-back, wrong-key/tamper rejection, clean-target preflight, fresh-process bundle restore, cleanup pass, and release-ledger scalar summaries in local fixture scope; provider-managed SSE/KMS, external backend, key custody, provider-pair, clean-host/disaster-recovery, and outage evidence remain open |
...
| Bounded object-store multipart upload/read-back | [`object_store_multipart.py`](mas/packages/mas-core/mas_core/memory/object_store_multipart.py), [`check_object_store_multipart.py`](mas/scripts/check_object_store_multipart.py), [`test_object_store_multipart.py`](mas/packages/mas-core/tests/test_object_store_multipart.py), [multipart evidence](mas/docs/provenance/object_store_multipart_provider_diverse_evidence.json), [storage feature](Docs/current/FEATURE_DATA_STORAGE_AND_MEMORY.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md); commit `a2f35de` | Compose MinIO and disposable SeaweedFS each pass bounded 8 MiB/16 MiB multipart payloads with 5 MiB parts, checksum read-back, explicit abort-without-object, payload-free reporting, and zero cleanup. Resource, provider outage, KMS, clean-host, disaster-recovery, and migration-cutover evidence remain open |
| Bounded object-store resource profile | [`object_store_resource_profile.py`](mas/packages/mas-core/mas_core/memory/object_store_resource_profile.py), [`check_object_store_resource_profile.py`](mas/scripts/check_object_store_resource_profile.py), [`test_object_store_resource_profile.py`](mas/packages/mas-core/tests/test_object_store_resource_profile.py), [scalar evidence](mas/docs/provenance/object_store_resource_profile_provider_diverse_evidence.json), [migration review](Docs/current/FEATURE_OBJECT_STORE_MIGRATION_STATUS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md); commits `3791b3f`/`bb0fa8f`/`c90fc13` | Fresh corrected Compose MinIO and disposable SeaweedFS 4.42 wave passes eight 1 MiB/8 MiB checksum cases per provider at concurrency four with identical procfs RSS/wall/CPU sampling, zero errors, and cleanup. The same reserved namespace passes 8 MiB/16 MiB multipart upload/read-back, 5 MiB-part abort, and zero cleanup. The prior nonexistent-alias attempt is excluded as harness/configuration-invalid; production resource budgets/portability, KMS, clean-host, disaster-recovery, and provider selection remain open; actual process outage/recovery is certified separately |
| Bounded object-store provider-process outage/recovery | [`check_object_store_provider_outage.py`](mas/scripts/check_object_store_provider_outage.py), [`test_check_object_store_provider_outage.py`](mas/scripts/tests/test_check_object_store_provider_outage.py), [scalar evidence](mas/docs/provenance/object_store_provider_outage_live_evidence.json), [release ledger](mas/docs/provenance/release_ledger.yaml); commits `7e22b06`/`f728c6f`/`d92b3dc`/`407a4f9` | Fresh labelled disposable MinIO and SeaweedFS containers on canonical `mas_internal` receive the same 64 KiB/1 MiB checksum workload, become unreachable during a controlled process stop, restart with disposable volumes, read back successfully, and clean to zero objects with containers/helpers/volumes removed. Earlier development attempts are excluded; provider-managed KMS, production resource thresholds, clean-host/disaster recovery, and independent-host evidence remain open |
| Optional memory/workflow service evaluation contract | [Data, Storage, Memory, and Retention](Docs/current/FEATURE_DATA_STORAGE_AND_MEMORY.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`optional_memory_services.yaml`](mas/docs/provenance/optional_memory_services.yaml), [`check_optional_memory_services.py`](mas/scripts/check_optional_memory_services.py), [`optional_memory_services_contract.json`](mas/docs/provenance/optional_memory_services_contract.json); commits `2760c7a`/`5d62ca6`/`0f1db19` | Static candidate contract and exact Qdrant/Temporal adapter contracts pass for Letta, Qdrant, and Temporal with three AIAT-owned stewards, six conformance checks and four measurable metrics each, opaque project namespaces, bounded inputs, scalar-only results, zero enabled candidates, no network/mutation, and scalar-only evidence. Live value, recovery, backup/restore, and removal tests remain open; candidates are not selected or release gates |
| AIAT credentials-manager secret boundary | [`check_credentials_manager_live.py`](mas/scripts/check_credentials_manager_live.py), [`test_check_credentials_manager_live.py`](mas/scripts/tests/test_check_credentials_manager_live.py), [scalar evidence](mas/docs/provenance/credentials_manager_live_evidence.json), [release ledger](mas/docs/provenance/release_ledger.yaml); commits `43ffcf5`/`12ba7c7`/`1808a15`/`d101901`/`a946368`/`f9527b3`/`a90fda2`/`5da200f`/`53660d3`/`4c6f400` | The correctly invoked private Compose probe (`--live --compose-local --json`) runs the production `CredentialsManager` against the internal Postgres hostname and passes all 12 checks (fresh scalar read-back 2026-08-21T17:49:10Z): ciphertext-at-rest, metadata redaction, policy denial, approved server-side resolution, one-use approval, rate-limit denial, audit persistence, and zero fixture residue. Key material, values, payloads, and credentials are excluded. This advances AIAT-owned secret management only; provider-managed SSE/KMS, external custody/rotation, clean-host bootstrap, and disaster recovery remain separate operator gates |
...
| Dashboard new-flow builder access-denied recovery | [Dashboard and Operator UX](Docs/current/FEATURE_DASHBOARD_AND_OPERATOR_UX.md); governed catalogue/create/readiness boundary `b07299b`, source-built `flow-builder-states.spec.ts` 3/3; native/live ACL and full WCAG/native-Linux evidence remain open |
...
| Dashboard Project Evidence access-denied recovery | [Dashboard and Operator UX](Docs/current/FEATURE_DASHBOARD_AND_OPERATOR_UX.md); canonical evidence-package read boundary `00f81b5`, source-built `project-evidence-states.spec.ts` 3/3; native/live ACL and project evidence remain open |
| Dashboard Evidence Detail access-denied recovery | [Dashboard and Operator UX](Docs/current/FEATURE_DASHBOARD_AND_OPERATOR_UX.md); bounded scalar-read boundary `23e2db9`, source-built `dashboard-evidence-detail.spec.ts` 11/11; native/live ACL and broader evidence certification remain open |
| Dashboard System Overview access-denied recovery | [Dashboard and Operator UX](Docs/current/FEATURE_DASHBOARD_AND_OPERATOR_UX.md); seven-source control-plane/metrics boundary `b0ab779`, source-built `system-overview-recovery.spec.ts` 1/1 against a local 403 fixture; native/live ACL and full WCAG/native-Linux evidence remain open |
...
| Concurrent multi-host native worker execution and duplicate-effect protection | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_multi_host_execution_postgres.py`](mas/scripts/check_worker_multi_host_execution_postgres.py), [local evidence](mas/docs/provenance/worker_multi_host_execution_postgres_evidence.json), commit `d45e4dd` |
| Local independent-process worker execution | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_independent_process_execution_postgres.py`](mas/scripts/check_worker_independent_process_execution_postgres.py), [`test_check_worker_independent_process_execution_postgres.py`](mas/scripts/tests/test_check_worker_independent_process_execution_postgres.py), [local evidence](mas/docs/provenance/worker_independent_process_execution_postgres_evidence.json), commits `cec6558` and `520c6bf` |
...
| Same-host worker-host loss/recovery soak | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_host_loss_queue_recovery_soak_postgres.py`](mas/scripts/check_worker_host_loss_queue_recovery_soak_postgres.py), [`test_check_worker_host_loss_queue_recovery_soak_postgres.py`](mas/scripts/tests/test_check_worker_host_loss_queue_recovery_soak_postgres.py), [local evidence](mas/docs/provenance/worker_host_loss_queue_recovery_soak_postgres_evidence.json); commit `424805c` repeats the production local recovery checker in three separate child processes with durable reopen, scalar-only projection, and zero-row cleanup. It is not independent-host, provider-outage, sandbox, deployment-load/chaos, or disaster-recovery evidence |
| Selected model-resolution worker-host execution, gateway worker, and terminal attribution | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_host_model_resolution_postgres.py`](mas/scripts/check_worker_host_model_resolution_postgres.py), [`check_gateway_worker_adapter.py`](mas/scripts/check_gateway_worker_adapter.py), [`check_gateway_worker_http_fixture.py`](mas/scripts/check_gateway_worker_http_fixture.py), [`test_gateway_worker_adapter.py`](mas/packages/mas-core/tests/test_gateway_worker_adapter.py), [`test_check_gateway_worker_http_fixture.py`](mas/scripts/tests/test_check_gateway_worker_http_fixture.py), [`test_worker_model_attribution.py`](mas/packages/mas-core/tests/test_worker_model_attribution.py), [`worker_manifest.py`](mas/packages/mas-core/mas_core/protocols/worker_manifest.py), [`runtime_catalog.py`](mas/packages/mas-core/mas_core/worker_registry/runtime_catalog.py), [`check_worker_reconciliation.py`](mas/scripts/check_worker_reconciliation.py), [host evidence](mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json), [gateway fixture evidence](mas/docs/provenance/gateway_worker_adapter_fixture.json), [gateway HTTP evidence](mas/docs/provenance/gateway_worker_http_fixture.json); `080ee18` adds the bounded AIAT gateway worker fixture, `f6baebc` registers its transport contract, `cec1e4c` hardens lifecycle/input bounds, `cbbfe56` certifies the real client HTTP boundary/retry path, `199eb5b` rejects missing/incomplete/mismatched provider/model usage before terminal evidence persistence, and `8ed53df` carries the production gateway adapter through durable Postgres host execution |
| Explicit live worker-plane provider boundary | [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_gateway_worker_provider_live.py`](mas/scripts/check_gateway_worker_provider_live.py), [`test_check_gateway_worker_provider_live.py`](mas/scripts/tests/test_check_gateway_worker_provider_live.py), [live evidence](mas/docs/provenance/gateway_worker_provider_live.json) | `f999695` adds a fail-closed, explicit-opt-in runner that reads `/v1/models`, requires a selected exact model and operator opt-in, and drives one bounded completion through the real host executor/controller/adapter chain with scalar secret-safe output. Retained certificate `90c3e5d` records one listed-model check and one successful `llama-3.3-70b-versatile` completion with `SUCCEEDED` settlement and binding/reservation release. Durable Postgres, independent-host, sandbox, recovery, and provider mail-edge evidence remain open |
| Local gateway-worker/mail-edge composition | [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [Trace Evidence and Retention](Docs/current/FEATURE_TRACE_EVIDENCE_AND_RETENTION.md), [Mail-Edge Observability](Docs/current/FEATURE_MAIL_EDGE_OBSERVABILITY.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_gateway_worker_mail_edge_fixture.py`](mas/scripts/check_gateway_worker_mail_edge_fixture.py), [`check_gateway_worker_mail_edge_postgres.py`](mas/scripts/check_gateway_worker_mail_edge_postgres.py), [`test_check_gateway_worker_mail_edge_fixture.py`](mas/scripts/tests/test_check_gateway_worker_mail_edge_fixture.py), [`test_check_gateway_worker_mail_edge_postgres.py`](mas/scripts/tests/test_check_gateway_worker_mail_edge_postgres.py), [fixture evidence](mas/docs/provenance/gateway_worker_mail_edge_fixture.json), [durable evidence](mas/docs/provenance/gateway_worker_mail_edge_postgres_evidence.json), [raw-provider evidence](mas/docs/provenance/gateway_worker_mail_edge_provider_postgres_evidence.json), [provider-shaped recovery evidence](mas/docs/provenance/gateway_worker_mail_edge_provider_recovery_postgres_evidence.json), [live evidence](mas/docs/provenance/gateway_worker_provider_mail_edge_live.json); commits `6ebb12c`, `fa42284`, `67f1599`, `0e0a76f`, `1679341`, and `9c7e76d` certify scalar and durable local composition plus signed delegated/raw Resend/Svix ingress, two-attempt local provider-shaped retry, exact fixture attribution, durable provider-message worker/trace correlation, replay/conflict/tamper checks, independent worker/identity reopen, payload-free correlation, and scoped cleanup; `17f6547` plus retained live certificate records one selected live worker/provider call, durable dual-Postgres reopen, raw-ingress delivered/bounced read-back, payload-redacted result persistence, and cleanup; external provider callback/delivery/outage recovery, independent host/process, and sandbox evidence remain open |
| Gateway worker failure classification | [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`runtime_adapters.py`](mas/packages/mas-core/mas_core/worker_registry/runtime_adapters.py), [`test_gateway_worker_adapter.py`](mas/packages/mas-core/tests/test_gateway_worker_adapter.py); commits `b2ae516`, `1679341`, and `9c7e76d` classify bounded input, transient gateway, permanent provider failures, and the durable local one-retry path with secret-safe metadata; external provider recovery remains open |
...
| Identity-provider mocked conformance | [`aiat.identity-provider-conformance.v1`](mas/scripts/check_identity_provider_conformance.py), [`test_check_identity_provider_conformance.py`](mas/scripts/tests/test_check_identity_provider_conformance.py), [`test_provider_adapters.py`](mas/apps/identity-service/tests/test_provider_adapters.py); `c568b87` |
...
| Flow execution semantics review status | [Flow Execution Semantics Status](Docs/current/FLOW_EXECUTION_SEMANTICS_STATUS.md), [`check_flow_execution_semantics.py`](mas/scripts/check_flow_execution_semantics.py), [`test_flow_execution_semantics.py`](mas/packages/mas-core/tests/test_flow_execution_semantics.py); switch-node activation/selection is covered by the live certificate below |
| Live flow runtime execution/recovery | [`check_flow_runtime_live.py`](mas/scripts/check_flow_runtime_live.py), [`test_check_flow_runtime_live.py`](mas/scripts/tests/test_check_flow_runtime_live.py), [`flow_engine.py`](mas/packages/mas-core/mas_core/workflow/flow_engine.py), scalar evidence [`flow_runtime_live_evidence.json`](mas/docs/provenance/flow_runtime_live_evidence.json), commit `27247f4` | Local Compose API certificate passes 12/12 parallel/join/switch/cancel/timeout/escalation/safe-retry cases and removes two flow plus three project fixtures with zero residue; native watchdog, cold-crash, worker-canary, sandbox, and provider recovery remain separate |
...
| Durable flow-instance recovery certificate | [`check_flow_instance_recovery_postgres.py`](mas/scripts/check_flow_instance_recovery_postgres.py), [`test_check_flow_instance_recovery_postgres.py`](mas/scripts/tests/test_check_flow_instance_recovery_postgres.py), [retained Postgres evidence](mas/docs/provenance/flow_instance_recovery_postgres_evidence.json), commit `1f20132` |
| Workflow watchdog and safe-recovery review status | [Workflow Watchdog and Safe-Recovery Status](Docs/current/WORKFLOW_WATCHDOG_RECOVERY_STATUS.md), [`check_workflow_watchdog_recovery.py`](mas/scripts/check_workflow_watchdog_recovery.py), [`test_workflow_watchdog_recovery.py`](mas/packages/mas-core/tests/test_workflow_watchdog_recovery.py), deterministic cancellation/escalation/timeout/fail-closed extension `9972b3b` |
...
| Machine-readable release evidence aggregation | [`check_release_ledger.py`](mas/scripts/check_release_ledger.py), [`check_release_environment.py`](mas/scripts/check_release_environment.py), [`check_operator_pins.py`](mas/scripts/check_operator_pins.py) (`dd857ae`), [`check_image_provenance.py`](mas/scripts/check_image_provenance.py) (`2804a9f` strict inventory), [`check_image_budgets.py`](mas/scripts/check_image_budgets.py), [`check_metric_series_budget.py`](mas/scripts/check_metric_series_budget.py), [`check_docs_index.py`](mas/scripts/check_docs_index.py), [release-ledger inventory](mas/docs/provenance/release_ledger.yaml) |
...
- read-only selected `aiat.worker-run-readiness.v1` preflight with explicit
  ...
  worker dispatch and certification remain separate gates. The live checker
  fails closed on unavailable or malformed health evidence with the stable
  `read_worker_health_unavailable` blocker (`2eea80a`);
...
- `scripts/check_release_environment.py` emits a secret-safe
  `aiat.release-environment.v1` manifest with fourteen release-input hashes,
  including the security-finding review register, tool-version identities,
  environment-presence flags, and a deterministic digest; dirty/frozen-
  worktree state remains explicit release evidence.

The exact OpenCode scan remains a technical blocker: `23e908e` adds a
machine-checked operator review register covering all 316 findings and 54
engine warnings, but it is explicitly not a waiver and the manifests remain
`findings_review_required` until technical dispositions and a passing rerun are
recorded. This roadmap therefore starts with integrity and certification gaps
rather than another architecture rewrite.
...
matching protocol provenance hash; the historical static ledger run is 52/52 pass
with two pending
...
- [x] deployed team runners use the authenticated control-plane storage API (`43bee16`, typed/fail-closed hardening `22fc21a`) for checkpoints, usage, documents, and COO review persistence without PgBouncer/MinIO/shared-service credentials or private data-plane network membership; checkpoint access is team-scoped, OpenCode is off the runner network, and startup fails closed when storage health is unavailable; the policy-backed matrix contract (`96fb71f`) validates the same protected/allowed rows in static and live modes, the refreshed local WSL2 matrix passes all 11 runners, and evidence is retained at [`provenance/network_boundary_live.json`](mas/docs/provenance/network_boundary_live.json), while native denial/allow evidence remains open;
...
- [x] the current WSL2 Docker engine reconciles all ten supplied immutable image references to matching local `RepoDigests` (`75f9be0`, scalar evidence at [`mas/docs/provenance/image_provenance_local_identity.json`](mas/docs/provenance/image_provenance_local_identity.json)); this is local identity evidence only and does not substitute for deployment-supplied SBOM, scan, source/lock, clean-build, or native release evidence;
...
blockers in the live preflight; follow-up `dac268c` also rejects a successful
but empty health payload. Fixture and live reads remain non-mutating.
...
- [x] Harden the selected worker-run readiness live read to fail closed on an
  unavailable or malformed health response (`2eea80a`, `dac268c`); emit the stable
...
Project-detail refresh failures are covered by the existing project workspace golden path: canonical project data remains visible, the page labels it stale, and an operator can retry without losing workspace context. Project-detail first-load failures now also show an explicit unavailable state with backend error detail and Retry before recovering into the workspace (`f364763`). A canonical project read or workflow/mutation 401/403 now renders a named denial region, retains only the last-known project header, and hides refresh, tabs, panels, and mutation controls; the source-built denial/recovery matrix passes 4/4 (`0671eaa`). The project evidence package now similarly fails closed on canonical package-read 401/403 responses, retaining only a last-known package and hiding Refresh/Retry/package controls; its source-built denial/recovery matrix passes 3/3 (`00f81b5`). Evidence Detail now fails closed on bounded scalar-read 401/403 responses, retains any last-known scalar projection, and hides Refresh while preserving citation navigation; its source-built denial/recovery matrix passes 11/11 (`23e2db9`). System Overview now distinguishes access-denied control-plane/metrics reads from outages, retains available overview values read-only, and hides retry and first-run seed actions; its local 403 fixture passes 1/1 (`b0ab779`). The new-flow builder now applies the same boundary to governed catalogue reads and create/readiness mutations, retaining the draft canvas read-only and hiding editing controls; its source-built denial matrix passes 3/3 (`b07299b`). The flow editor now has the same bounded load/recovery contract: first-load failures are explicit and retryable, while refresh failures retain the last-known canvas (`b5098e7`); its 401/403 canonical read/save boundary preserves only that canvas as read-only and hides editor mutations (`392d264`).
...
- [x] deterministic watchdog/recovery semantics are covered by
  ...
  watchdog failure, recorded-safe-state retry, terminal-state exclusion,
  explicit cancellation, review-circuit escalation, node-timeout failure, and
  invalid-transition rejection; native watchdog/cold-crash proof remains;
...
matrix (`392d264`).
The new-flow builder now separately proves its initial catalogue, create, and
readiness-validation denial states: the draft canvas remains read-only while
templates, palette, node configuration, validation, activation, and creation
controls are hidden (`b07299b`, source-built matrix 3/3).
...
partial fixture runs pass 1/1 (`50cee61`). Its 401/403 access-denied branch
identifies denied sources, retains available values as read-only, and hides
retry and first-run seed actions; the isolated local 403 fixture passes 1/1
(`b0ab779`). The shared `EmptyState` primitive
...
covered by the following groups, and the scheduler group below proves bounded
multi-host selection/fallback. Host fencing and expired-host recovery are now
covered by `72e59ec`; worker dispatch and Firecracker evidence remain open.
...
connection reopen while an expired lease becomes ineligible. Host fencing and
expired-host recovery are covered by `72e59ec`; capacity reservation/commit is
covered by `232c0bb` and scheduler integration by `d9917f8`.
...
scalar capacity read-back. Reservations now retain the host lease generation
that created them. Scheduler selection/fallback is covered by `d9917f8`;
host-fencing/recovery is covered by `72e59ec`; Firecracker evidence remains
open.

`d9917f8` adds the `aiat.worker-host-scheduler.v1` integration over the
registry, placement predicate, and reservation ledger. It deterministically
tries the preferred host, falls back after a row-locked capacity rejection,
replays a globally idempotent schedule key, filters draining/unleased hosts,
reports blocked full capacity, survives connection reopen, and performs
scoped cleanup without worker/provider dispatch. Host-loss/split-brain fencing
and recovery are covered by `72e59ec`; live dispatch and Firecracker evidence
remain open.

`72e59ec` adds migration `0039_worker_host_fencing` and
`aiat.worker-host-recovery.v1`. Re-registration advances a durable host lease
generation and expires reservations from the old incarnation; the bounded
recovery reconciler fences expired READY/DRAINING leases to `OFFLINE`, expires
current-generation reservations in the same transaction, rejects stale
heartbeats, and leaves the recovered host ineligible to placement after a
connection reopen. The retained certificate is
...
`08f1610e` adds migration `0042_worker_run_host_binding` and the durable
`aiat.worker-run-host-binding.v1` service. A binding checks the run/worker
identity, reuses the scheduler's worker-plane reservation, copies its host
lease generation, replays an assignment key, and settles owner-bound commit or
release transitions. The local certificate at
[`worker_run_host_binding_postgres_evidence.json`](mas/docs/provenance/worker_run_host_binding_postgres_evidence.json)
survives connection reopen and cleans its fixture graph; it is assignment
authority only, so live worker/provider/sandbox execution remains open.

The current working tree narrows the settlement window: normal reservation and
run-host binding commit/release now use the same database transaction through
the reservation ledger's in-transaction primitive. A failed new assignment,
including a host-loss replacement, releases the newly-created reservation,
while replayed reservations are left untouched. Host-loss replacement binding
and replacement-reservation commit now share one transaction after scheduling;
reservation creation, worker claims, evidence/usage settlement, and live host
recovery still need explicit reconciliation and fault evidence.
...
fixture and guarded disposable live rehearsal pass the full sequence without
changing deployment routing or retention authority. The live evidence is
reserved-prefix-only and does not authorize production cutover.
...
Commit `351444a` adds the bounded `aiat.object-store-provider-pair.v1` live
certificate at
[`mas/docs/provenance/object_store_provider_pair_evidence.json`](mas/docs/provenance/object_store_provider_pair_evidence.json).
It dual-writes three checksum-bearing objects between the existing Compose
MinIO endpoint and a disposable MinIO endpoint, rejects the simulated primary
adapter failure, restores from the secondary into a clean recovery bucket,
and cleans every reserved prefix. Both endpoints are MinIO; provider-diverse
durability, actual provider process/network outage, KMS, clean-host, and
...
resource, outage/recovery, encrypted/provider backup,
clean-environment restore, provider-diverse migration, and multi-host proof
remain open. The retained MinIO runs are local deployment evidence, not a claim
about another provider or disaster recovery.
The provider-diverse follow-up in `f52fde6` now retains scalar-only
MinIO/SeaweedFS endpoint comparison and pair-recovery evidence at
[`mas/docs/provenance/object_store_provider_benchmark_evidence.json`](mas/docs/provenance/object_store_provider_benchmark_evidence.json)
and
[`mas/docs/provenance/object_store_provider_pair_provider_diverse_evidence.json`](mas/docs/provenance/object_store_provider_pair_provider_diverse_evidence.json).
...
recovery claim.
Commit `6794b9f` extends the benchmark with a bounded large-object/concurrency
wave: four concurrent 1 MiB cases and four concurrent 8 MiB cases pass on each
endpoint with checksum read-back and zero remaining fixture objects. Scalar
...
Resource, outage, provider-managed encryption/KMS, clean-host,
disaster-recovery, and provider-selection evidence remain open.
...
scoped cleanup. Commit `b0f27f6` and evidence group `59294c0` add the local
clean-process prerequisite at
[`mas/scripts/check_object_store_clean_environment_restore.py`](mas/scripts/check_object_store_clean_environment_restore.py):
a ciphertext/scalar-manifest bundle is reopened by a distinct Python process
with fresh adapters, verified through the production encrypted-restore helper,
and removed. Provider-managed SSE/KMS, external backend durability, key
custody/rotation, provider-pair, clean-host/disaster-recovery, and outage
...
`1d8aed5` adds `aiat.worker-mail-edge-coverage.v1` and
[`check_worker_mail_edge_coverage.py`](mas/scripts/check_worker_mail_edge_coverage.py),
which require explicit worker/trace scope and join worker source counts to
verified delivery/bounce observations without selecting or dispatching a
worker. The deterministic certificate is retained at
[`worker_mail_edge_coverage_fixture.json`](mas/docs/provenance/worker_mail_edge_coverage_fixture.json);
live worker/provider/bounce evidence remains open.
`acd3f06` adds [`check_worker_run_postgres_evidence.py`](mas/scripts/check_worker_run_postgres_evidence.py),
which drives the real controller/native adapter against local Compose Postgres,
reopens the store, verifies durable lifecycle, usage/artifact, and native
model/worker/audit trace rows without exposing task/result payloads, and cleans
only its reserved fixture namespace. Evidence is retained at
...
`08f1610e` adds migration `0042_worker_run_host_binding` and the
`aiat.worker-run-host-binding.v1` service. The durable assignment binds a
Worker Run to the selected worker-plane reservation, preserves its lease
generation, replays assignment keys, and settles owner-bound commit/release
transitions after connection reopen. The certificate is assignment authority
only; live worker/provider/sandbox execution remains open.
...
operator-selected worker and trace. Commit `cfafe38` adds identity-service
migration `0003_mail_edge_observations`, signed delegated webhook ingestion,
Resend normalization, idempotent/conflict-preserving persistence, and scalar
`mail-relay` dashboard/trace/SLO projection. Live mode still reports
`attention` until a deployed provider adapter, selected worker, and bounce
read-back supply live evidence.
Commit `aab6285` adds the real local ASGI ingress certificate and
[`check_mail_edge_ingress.py`](mas/scripts/check_mail_edge_ingress.py), with
secret-safe evidence at
[`mas/docs/provenance/mail_edge_ingress_certification.json`](mas/docs/provenance/mail_edge_ingress_certification.json).
It intentionally leaves the live provider callback, Postgres durability,
selected worker, and bounce read-back gate open.
Commit `2d04b30` adds the durable local Postgres ingress certificate and fixes
the identity image build context so the current `mas-core` package and
`0003_mail_edge_observations` migration are included in rebuilt local images.
It verifies connection reopen and payload-free dashboard read-back, then
cleans the reserved fixture namespace; deployed provider/worker/bounce and
outage/restore evidence remain open.
...
- [x] deterministic checksum manifest plus source → backup → clean-target
  ...
  provider, encryption, retention, and provider-backed clean-environment
  evidence exists;
...
- [x] fail closed on a non-empty restore prefix before copy and retain the
  `clean_target_verified` restore evidence (`93bf755`); provider-backed,
...
- [x] implement the AIAT-owned AES-256-GCM encrypted backup envelope and
  ...
  key-custody, provider-pair, clean-host/disaster-recovery, and outage evidence
  remain separate;
- [x] certify local clean-process encrypted bundle restore (`b0f27f6`, evidence
  `59294c0`) through a distinct Python process, fresh object-store adapters,
  production encrypted read-back/replication, scalar-only evidence, and
  temporary-bundle/fixture cleanup; clean-host, provider-pair, KMS, outage,
  and disaster-recovery evidence remain separate;
...
- [x] guarded live migration rehearsal across disposable MinIO/SeaweedFS
  (`ecbef00`) with explicit reserved-prefix cleanup and scalar evidence;
  production routing, retention, outage, KMS, clean-host, and disaster-recovery
...
- [x] operator-only incident API, generated contracts, dashboard proxy, and
  `/logs?trace_id=…` deep link (`b4b7cef`); the summary remains payload-free
  and non-gating while live incident population and richer chronology remain
...
- [x] payload-free `aiat.mail-edge-observation.v1`/`aiat.mail-edge-coverage.v1`
  ...
  and deterministic/fail-closed checker (`85369fe`);
- [x] identity-service migration `0003_mail_edge_observations`, signed
  delegated provider-webhook persistence, normalized event-ID idempotency and
  conflict rejection, and scalar `mail-relay` dashboard/trace/SLO projection
  (`cfafe38`); configured provider callback and selected-worker live evidence
  remain separate;
- [x] Add Resend/Svix raw-body verification with a bounded timestamp tolerance
...
- [x] Add optional signed identity dashboard read-back, selected-trace
  filtering, and duplicate-event preference to the checker (`074ef8a`);
  configured provider callback, selected worker, and durable bounce evidence
  remain separate;
- [x] Join worker trace source coverage with explicitly scoped, payload-free
  mail-edge observations under `aiat.worker-mail-edge-coverage.v1` (`1d8aed5`);
  the deterministic certificate covers worker usage/artifact/model/worker and
  optional integration sources plus verified delivery/bounce signals without
  network or state mutation. Live worker/provider/bounce evidence remains
  separate;
- [x] Compose the durable worker/mail-edge certificate with the raw
  Resend/Svix provider-facing route (`0e0a76f`); the checker derives
  worker/trace scope from a durable provider-message delivery attempt and
  passes exact raw-body, replay/conflict/tamper, dual-Postgres reopen,
  payload-free join, and scoped-cleanup checks. External provider callback,
  delivery, recovery, selected worker, and sandbox evidence remain separate;
...
- provider-backed encrypted Garage/R2/B2 or approved backup and clean-host/
...
9. [x] Codify the static/live network boundary verifier and remove the OpenCode runtime from the runner network (`43bee16`). [x] Move the deny/allow rows into `aiat.network-boundary-policy.v1` and make static/live checks consume them (`96fb71f`). [x] Recreate the local stack and pass the 11-runner denial/allow matrix; [ ] repeat on a native-Linux release host.
...
5. Finish model/budget/executive paths; focused override-expiry normalization,
 ...
 read-only `/executive/views/{role}` endpoints, reservation/settlement
   ...
   explicit stale/retry recovery (`5f4b0eb`) and fails closed on denied reads
   while preserving last-known tables/details (`b418f8a`); the dead-letter queue retains its
   last known messages (`823fa6d`) and fails closed on denied read/replay
   responses while preserving only last-known messages (`e6ab3a1`), the
   credentials list retains redacted
   ...
   reads and exposes a bounded GET retry (`50cee61`), while its 401/403
   access-denied branch retains available values read-only and hides retry and
   first-run seed actions (`b0ab779`); the
   Flows list retains definitions through
   failed refreshes (`a0faf5b`) and now fails closed on denied reads/deletes
   while preserving only read-only last-known definitions (`3108b02`); the flow editor exposes explicit first-load
...
2. [x] Define the disabled-by-default optional memory/workflow service
   catalogue and scalar contract (`2760c7a`/`5d62ca6`); [ ] run live
   value, outage, backup/restore, and removal evidence only after explicit
   operator endpoints, versions, budgets, project scope, and certified
   sandboxes are supplied.
