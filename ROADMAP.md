...
The latest grouped verification (`b141c02`, based on documentation commit
`5616bae`, 2026-08-18) keeps the static release ledger at 48/48 pass and
confirms the protocol, documentation-index, provenance, and host-restart-
boundary checks. The selected live provider certificate is now retained, but
this does not advance R1 to release-ready: the native host, durable worker,
sandbox, pending security review, and clean-worktree gates remain open. The
durable selected-model host certificate was then tightened by `8ed53df` to execute the production
`GatewayWorkerAdapter` through a bounded local gateway fixture and refresh its
Postgres evidence; this does not close the live provider or sandbox gates.
...
matching protocol provenance hash; the latest static ledger run is 50/50 pass
with two pending
