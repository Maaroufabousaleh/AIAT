...
The latest grouped verification through `b1028d6` (including the bounded
same-host recovery soak from `424805c` and synchronized documentation group
`06393d1`; based on the workflow-control fixture extension, the durable local
...
the existing network/sandbox evidence; current ledger snapshot `b1028d6`,
2026-08-18) keeps
...
| Bounded object-store provider-pair recovery | [Data, Storage, Memory, and Retention](Docs/current/FEATURE_DATA_STORAGE_AND_MEMORY.md), [Object-Store Migration Review Status](Docs/current/FEATURE_OBJECT_STORE_MIGRATION_STATUS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_object_store_provider_pair.py`](mas/scripts/check_object_store_provider_pair.py), [`test_check_object_store_provider_pair.py`](mas/scripts/tests/test_check_object_store_provider_pair.py), [live evidence](mas/docs/provenance/object_store_provider_pair_evidence.json); commit `351444a` and evidence `f385bd7` certify three-object checksum dual-write between two local MinIO endpoints, adapter-boundary primary-loss rejection, secondary-only clean recovery, payload-free output, and zero cleanup. Provider-diverse durability, actual provider outage, KMS, clean-host, and disaster-recovery evidence remain open |
