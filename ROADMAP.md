...
| --- | --- | --- |
| R0 — documentation authority | in progress | Canonical target, eleven feature documents, three plans, root navigation, and the personal/internal metadata-only policy are present. Both the current workspace and a clean Git archive pass `check_docs_index.py`; the generated/ignored `mas/uv.lock` remains a local runtime input rather than a documentation link. |
...
| Default runtime packaging contract | [`check_runtime_install_profile.py`](mas/scripts/check_runtime_install_profile.py), [`pyproject.toml`](mas/apps/orchestrator-api/pyproject.toml), the locally generated `uv.lock`, and [orchestrator Dockerfile](mas/infra/docker/Dockerfile.orchestrator-api) |
