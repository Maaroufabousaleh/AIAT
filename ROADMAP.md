...
The follow-on durable composition certificate `fa42284` now runs the same
production gateway adapter/controller against the worker Postgres store and
the normalized identity mail-edge store, reopens both independently, verifies
exact fixture attribution and payload-free worker/mail-edge correlation, and
leaves zero reserved rows after cleanup. It does not claim HTTP provider
ingress, external provider delivery, selected live worker execution, recovery,
or sandbox evidence.
...
`fa42284` extends that composition to a durable dual-Postgres certificate. The
...
independently; and the cross-store evaluator passes before scoped cleanup.
...
This is local normalized-store composition only: HTTP provider ingress,
external provider delivery/recovery, selected live worker execution, and
