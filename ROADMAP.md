...
| R1 — P0 release integrity | in progress | Static ledger: 52/52 pass, including the Firecracker worker-pool readiness, bounded gateway-provider-recovery, security-finding-review, and multipart object-store children; release-ledger regression coverage asserts the registered children remain present and passing (`7f2ac9c`, `a864970`, `23e908e`, `424d310`). The policy-backed network matrix contract (`96fb71f`) now feeds the same protected/allowed rows to static and live checks, the OpenCode Compose sandbox contract is statically enforced (`2c098f5`), and the refreshed local WSL2 run passes 11/11 runners. The current unconfigured local live ledger records 55 pass, 0 fail, 16 blocked, 4 pending, and `NO-RELEASE` across 71 checks; the native release-host preflight, Firecracker host-readiness block, multipart live configuration block, and technical security gate remain explicit in that aggregate. The retained configured loopback profile remains 59 pass, 0 fail, 5 blocked, 4 pending across 64 checks. Native-Linux, deployment-wide trace/tool coverage, operator-selected worker certification, deployment image digests/SBOM/scan artifacts, gVisor, provider/mail, clean-worktree, unresolved OpenCode findings, and selected self-improvement evidence remain open. |
...
This closes only the bounded large-object/concurrency comparison; resource,
outage, provider-managed encryption/KMS, clean-host, and
...
scope: serial small-object baselines and one bounded larger/concurrent wave are
...
2026-08-18T14:08:29Z) keeps
the static release ledger at 52/52 pass and the
unconfigured local aggregate at 55 pass, 0 fail, 16 blocked, and 4 pending with
...
ledger at 52/52 pass and confirms the protocol, documentation-index,
...
matching protocol provenance hash; the latest static ledger run is 52/52 pass
with two pending
