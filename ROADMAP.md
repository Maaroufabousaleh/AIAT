...
| Selected model-resolution worker-host execution and terminal attribution | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_host_model_resolution_postgres.py`](mas/scripts/check_worker_host_model_resolution_postgres.py), [`test_worker_model_attribution.py`](mas/packages/mas-core/tests/test_worker_model_attribution.py), [local evidence](mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json); `199eb5b` rejects missing/incomplete/mismatched provider/model usage before terminal evidence persistence |
