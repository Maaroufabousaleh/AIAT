...
| Selected model-resolution worker-host execution | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_host_model_resolution_postgres.py`](mas/scripts/check_worker_host_model_resolution_postgres.py), [local evidence](mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json) |
...
| Model-profile catalogue and read-only gateway route | [`/model-profiles/catalogue`](mas/apps/orchestrator-api/orchestrator_api/main.py), [Governance proxy](mas/apps/mas-dashboard/app/api/governance/model-profiles/catalogue/route.ts), [`test_model_profile_catalogue.py`](mas/apps/orchestrator-api/tests/test_model_profile_catalogue.py), and refreshed [`model_profile_catalogue_live.json`](mas/docs/provenance/model_profile_catalogue_live.json) (`68e0b03`: 92/94 approved coverage, nine `/v1/models` entries, all five AIAT aliases; no dispatch/provider call) |
