...
| Bounded object-store provider-pair recovery | [Data, Storage, Memory, and Retention](Docs/current/FEATURE_DATA_STORAGE_AND_MEMORY.md), [Object-Store Migration Review Status](Docs/current/FEATURE_OBJECT_STORE_MIGRATION_STATUS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_object_store_provider_pair.py`](mas/scripts/check_object_store_provider_pair.py), [`test_check_object_store_provider_pair.py`](mas/scripts/tests/test_check_object_store_provider_pair.py), [same-provider evidence](mas/docs/provenance/object_store_provider_pair_evidence.json), [provider-diverse evidence](mas/docs/provenance/object_store_provider_pair_provider_diverse_evidence.json), [benchmark evidence](mas/docs/provenance/object_store_provider_benchmark_evidence.json); commits `351444a`/`f385bd7`/`3833631` plus `f52fde6` certify three-object checksum dual-write between local MinIO/SeaweedFS endpoints, adapter-boundary primary-loss rejection, secondary-only clean recovery, payload-free output, six bounded benchmark cases, and zero cleanup. Provider durability/custody, actual provider outage, KMS, clean-host, disaster-recovery, large-object/multipart, and migration cutover evidence remain open |
