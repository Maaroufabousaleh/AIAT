...
Commits `080ee18`, `f6baebc`, and `cec1e4c` add the bounded `aiat_gateway` model-worker edge:
...
read-only reconciliation checker. The retained fixture evidence records no
external provider call, network mutation, or sandbox execution; live
provider-backed dispatch and hardened sandbox evidence remain open. The owned
gateway client is started/stopped by the adapter, and prompt/message batches
and generation temperature are bounded before dispatch.
...
| Selected model-resolution worker-host execution, gateway worker, and terminal attribution | [Worker-Plane Host Execution](Docs/current/FEATURE_WORKER_HOST_EXECUTION.md), [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_worker_host_model_resolution_postgres.py`](mas/scripts/check_worker_host_model_resolution_postgres.py), [`check_gateway_worker_adapter.py`](mas/scripts/check_gateway_worker_adapter.py), [`test_gateway_worker_adapter.py`](mas/packages/mas-core/tests/test_gateway_worker_adapter.py), [`test_worker_model_attribution.py`](mas/packages/mas-core/tests/test_worker_model_attribution.py), [`worker_manifest.py`](mas/packages/mas-core/mas_core/protocols/worker_manifest.py), [`runtime_catalog.py`](mas/packages/mas-core/mas_core/worker_registry/runtime_catalog.py), [`check_worker_reconciliation.py`](mas/scripts/check_worker_reconciliation.py), [host evidence](mas/docs/provenance/worker_host_model_resolution_postgres_evidence.json), [gateway fixture evidence](mas/docs/provenance/gateway_worker_adapter_fixture.json); `080ee18` adds the bounded AIAT gateway worker fixture, `f6baebc` registers its transport contract, `cec1e4c` hardens lifecycle/input bounds, and `199eb5b` rejects missing/incomplete/mismatched provider/model usage before terminal evidence persistence |
