...
covered by the following groups, and the scheduler group below now proves
bounded multi-host selection/fallback while host-loss/split-brain, worker
dispatch, and Firecracker evidence remain open.
...
recovery, host-loss/split-brain, worker dispatch, and Firecracker evidence
remain open; capacity reservation/commit is covered by `232c0bb` and scheduler
integration by `d9917f8`.
...
scalar capacity read-back. Scheduler selection/fallback is covered by
`d9917f8`; durable host-loss recovery, split-brain avoidance, and Firecracker
...
scoped cleanup without worker/provider dispatch. Host-loss/split-brain,
live dispatch, and Firecracker evidence remain open.
...
3. [x] Certify the bounded local Postgres worker lease/recovery boundary
   ...
   and host capacity reservation/settlement (`232c0bb`), plus canonical
   multi-host selection/scheduling (`d9917f8`); [ ] add complete bundle
   pinning, durable host-loss/split-brain recovery, live worker dispatch, and
