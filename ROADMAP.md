...
| --- | --- | --- |
| R0 — documentation authority | in progress | Canonical target, eleven feature documents, three plans, root navigation, and the personal/internal metadata-only policy are present. The current workspace passes `check_docs_index.py`; the clean archive check is kept explicit because the generated/ignored `mas/uv.lock` is not part of a detached Git archive. |
