...
| R1 — P0 release integrity | in progress | Static ledger: 51/51 pass, including the Firecracker worker-pool readiness, bounded gateway-provider-recovery, and security-finding-review children; release-ledger regression coverage asserts all three children remain present and passing (`7f2ac9c`, `a864970`, `23e908e`). The policy-backed network matrix contract (`96fb71f`) now feeds the same protected/allowed rows to static and live checks, the OpenCode Compose sandbox contract is statically enforced (`2c098f5`), and the refreshed local WSL2 run passes 11/11 runners. The current unconfigured local live ledger records 54 pass, 0 fail, 15 blocked, 4 pending, and `NO-RELEASE` across 69 checks; the native release-host preflight, Firecracker host-readiness block, and technical security gate remain explicit in that aggregate. The retained configured loopback profile remains 59 pass, 0 fail, 5 blocked, 4 pending across 64 checks. Native-Linux, deployment-wide trace/tool coverage, operator-selected worker certification, deployment image digests/SBOM/scan artifacts, gVisor, provider/mail, clean-worktree, unresolved OpenCode findings, and selected self-improvement evidence remain open. |
...
This closes only the bounded large-object/concurrency comparison; multipart,
resource, outage, provider-managed encryption/KMS, clean-host, and
...
matching protocol provenance hash; the latest static ledger run is 51/51 pass
with two pending
...
Multipart, resource, outage, provider-managed encryption/KMS, clean-host,
