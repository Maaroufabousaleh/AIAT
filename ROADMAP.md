...
The latest configured Compose release aggregation (`2026-08-18T20:18:06Z`,
revision `2bc7ca5`) remains 75 pass/0 fail/6 blocked across 81 checks with
...
aggregate at `2026-08-18T20:18:06Z` records 75 pass, 0 fail, 6 blocked, and
4 pending across 81 checks at revision `2bc7ca5`. It remains `NO-RELEASE`
...
profile versions out of 94 persisted versions, with the explicit
`aiat/omniroute-coding` identity reconciled to the canonical
`litellm/omniroute-coding` registry entry and one operator-owned stale profile
finding remaining; provider-specific live recovery,
...
5. Finish model/budget/executive paths; focused override-expiry normalization,
 ...
 read-only `/executive/views/{role}` endpoints, reservation/settlement
   ...
   coverage is 93/94 approved covered versions with no pending registered model;
   the explicit OpenCode/LiteLLM identity alias is reconciled and one
   operator-owned stale row is retained as a finding; provider-specific live recovery,
