...
| R1 — P0 release integrity | in progress | Static ledger: 55/55 pass, including the Firecracker worker-pool readiness, bounded gateway-provider-recovery, security-finding-review, multipart, object-store resource-profile, object-store provider-outage, and AIAT credentials-manager children; release-ledger regression coverage asserts the registered children remain present and passing (`7f2ac9c`, `a864970`, `23e908e`, `424d310`, `bb0fa8f`, `407a4f9`, `d101901`, `8dbf508`). The policy-backed network matrix contract (`96fb71f`) now feeds the same protected/allowed rows to static and live checks, the OpenCode Compose sandbox contract is statically enforced (`2c098f5`), and the refreshed local WSL2 run passes 11/11 runners. The unconfigured local live ledger remains 58 pass, 0 fail, 19 blocked, 4 pending, and `NO-RELEASE` across 77 checks; the refreshed configured Compose aggregation at 2026-08-18T17:24:47Z records 70 pass, 0 fail, 7 blocked, and 4 pending across the same 77 checks. The provider-diverse multipart/resource/outage certificates are now validated through the retained-live evidence path; trace transport/tool, metrics-series, worker-reconciliation, runtime-benchmark, model-catalogue, and credentials evidence remains scalar-only. The OpenCode/LiteLLM profile identity reconciliation is fixed, and one operator-owned stale profile row remains visible. SLO remains descriptive/attention because native sources are not observed. Native-Linux, operator-selected worker certification, deployment image digests/SBOM/scan artifacts, gVisor, Firecracker, provider/mail, clean-worktree, unresolved OpenCode findings, external KMS/key custody, clean-host/disaster-recovery, and selected self-improvement evidence remain open. |
