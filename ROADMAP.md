...
The latest grouped verification (`c10ec59`, 2026-08-18) keeps the static
...
| Local gateway-worker/mail-edge composition | [Worker feature specification](Docs/current/FEATURE_WORKERS_STEWARDS_AND_MODELS.md), [Trace Evidence and Retention](Docs/current/FEATURE_TRACE_EVIDENCE_AND_RETENTION.md), [Mail-Edge Observability](Docs/current/FEATURE_MAIL_EDGE_OBSERVABILITY.md), [P2 scale plan](Docs/current/plans/P2_SCALE_STORAGE_AND_AUTONOMY_PLAN.md), [`check_gateway_worker_mail_edge_fixture.py`](mas/scripts/check_gateway_worker_mail_edge_fixture.py), [`test_check_gateway_worker_mail_edge_fixture.py`](mas/scripts/tests/test_check_gateway_worker_mail_edge_fixture.py), [evidence](mas/docs/provenance/gateway_worker_mail_edge_fixture.json); commit `6ebb12c` certifies local real-controller composition with scalar worker/mail-edge evaluators only; external provider, durable callback/read-back, live worker, and sandbox evidence remain open |
